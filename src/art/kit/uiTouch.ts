 ...
 *
 * 约定:
 * - 可点触区最小高度 40px(MIN_TOUCH_PX);过窄按钮补最小宽度 44px(MIN_TOUCH_WIDE_PX)。
 * - 正文类字号最低 14px(MIN_BODY_FONT_PX);纯装饰记号(图案角标、共享壳层 l99-star)
 *   豁免但需在 QA 报告登记。
 * - 只输出附加 CSS(min-height / min-width / font-size),追加在各款 CSS 末尾生效;
 *   不改布局模型、不动任何热区判定与宽度既有规则(如 bubble-pop 宽屏 .bp-cell 36px)。
 */

export const MIN_TOUCH_PX = 40;
...
/** 正文字号抬升:给正文类选择器统一到 ≥14px(追加在款内 CSS 末尾,可覆盖窄屏 media 的更小值) */
