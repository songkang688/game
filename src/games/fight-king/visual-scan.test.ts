/**
 * 朵星格斗王 · 1.3 窗口3 第 1 轮视觉验收 · 测试员机器扫描（静态源码断言）。
 *
 * 沉淀给第 2、3 轮复跑的三条底线：
 *  1. 核心资产模块 art.ts 里 fillText 不许直出 emoji（宪法负面清单①：裸 emoji 当核心资产）；
 *  2. art.ts 保有行头明暗双色（gearDark）与地面纵深带（drawGroundBands）——
 *     本款三阶光影走「look 表预置亮暗对」路线，纵深走 2.5D 地面渐变带，哨兵盯这两件；
...
describe("1.3 视觉机器扫描（窗口3 · round1 tester）", () => {
  ...
  it("art.ts 保有行头明暗双色与地面纵深带（体积/2.5D 哨兵）", () => {
    const src = read("./art.ts");
    expect(src.includes("gearDark")).toBe(true);
    expect(src.includes("drawGroundBands")).toBe(true);
  });
