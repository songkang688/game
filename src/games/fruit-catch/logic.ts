...
export class Janitor {
  ...
  pending(): number {
    return (
      this.timers.size + this.frames.size + this.heldTimers.length + this.heldFrames.length + this.offs.length
    );
  }
...
/**
 * 还活着的总管。闯关、双人、无尽各建各的，外壳只认 `mount()` 返回的那一对
