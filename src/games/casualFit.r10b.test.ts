...
describe("C-8 / N-45 补帐", () => {
  ...
  it("gold-hook 商店货架自滚、接着挖钉 footer", () => {
    expect(read("./gold-hook/style.ts")).toContain(".gdh-shopfoot{position:sticky;bottom:0");
    expect(read("./gold-hook/index.ts")).toContain("foot.append(purse, close)");
    expect(read("./gold-hook/index.ts")).toContain('veil.classList.add("gdh-shop")');
  });
