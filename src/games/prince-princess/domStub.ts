/**
 * 王子公主大冒险 · 测试用的极简 DOM 桩。
 *
 * 仓库的 vitest 跑在 node 环境(没有 jsdom,也不许为此引依赖),
 * 所以本款自己搭一份桩,只实现运行时真正用到的那几样:建节点、挂 / 摘监听、
 * 排 rAF、`window.setTimeout`、虚拟方向键用的 pointer 事件、
 * canvas 的 2d 画笔(画了什么都记下来 —— 规范表那六样形状是不是真画出来了,靠它数)、
 * 键盘的 `code`(两套键位认的是 code 不是 key)、
 * `localStorage`(188 关星级存档与城堡塔层数)与 `location.search`(`?level=` 直达)。
 *
 * 这个文件不带 `.test.` 后缀,vitest 不会把它当用例文件;
 * 玩法代码一行都没 import 它,打包时也不会被带进游戏 chunk。
 * 结构照搬同一仓库里已经用熟的那一份(`ice-fire-forest/domStub.ts`),
 * 免得每一款都重新发明一遍轮子。
 *
 * 比那一份多记两样东西,因为本款的画面是**照规范表画出来的**:
 * `FakeCtx` 把 `fillStyle` / `strokeStyle` / `lineWidth` 每次成型(fill / stroke)时的取值
 * 也存进 `ops`,用例才好断言「危险真的是那个描边色」「只有可踩的东西有亮顶边」。
 */
...
export interface DrawOp {
  op: "arc" | "ellipse" | "fillText" | "setLineDash" | "fill" | "stroke" | "fillRect";
  args: number[];
  text?: string;
  /** 成型那一刻的填色 */
  fill?: string;
  ...
  stroke?: string;
  lineWidth?: number;
  /** 这一笔的路径拐了几个点(三角形是 3,横条走 roundRect 记 0) */
  points?: number;
  /** 这一笔是虚线吗 */
  dashed?: boolean;
}
...
export class FakeCtx {
  ...
  fillRect(x: number, y: number, w: number, h: number): void {
    this.ops.push({ op: "fillRect", args: [x, y, w, h], fill: String(this.fillStyle) });
  }
  strokeRect(): void {}
  beginPath(): void {
    this.path = [];
  ...
  arc(x: number, y: number, r: number): void {
    this.ops.push({ op: "arc", args: [x, y, r], fill: String(this.fillStyle) });
  }
  arcTo(): void {}
  ellipse(x: number, y: number, rx: number, ry: number): void {
    this.ops.push({ op: "ellipse", args: [x, y, rx, ry] });
  }
  roundRect(): void {}
  rect(): void {}
  fill(): void {
  ...
  fillText(text: string, x: number, y: number): void {
    this.ops.push({ op: "fillText", args: [x, y], text });
  }
  strokeText(): void {}
...
/** 这棵树上第一块画布的画笔(规范表用例要翻它画了些什么) */
export function findCtx(root: FakeEl): FakeCtx | null {
  let hit: FakeCtx | null = null;
  walk(root, (el) => {
    if (!hit && el.tagName === "canvas") hit = el.getContext("2d");
  });
  return hit;
}
