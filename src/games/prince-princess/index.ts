...
const GUIDE_TIPS: string[][] = [
  [
    ...
    "跳到果冻怪头上踩一下也能把它弹开,而且会顺势弹得很高。",
    "两个人共用一条心条,一个人挨了打另一个人也会一起无敌一小会儿。",
  ],
  [
    "蝙蝠飞得高,王子要跳起来才砍得到,交给公主省事得多。",
    "公主在半空中还能再蹬一下(二段跳),够高台就靠它。",
    "空中平台上的宝石顺路捡,三星里有一颗看的就是宝石数。",
  ],
  [
    "铠甲怪头顶有一个 ⚔️,意思是「只有剑打得动」,星星会被弹开。",
    "炮台吐弹有节奏,数着它的间隔冲上去最安全。",
  ...
  [
    ...
    "火山这一章怪多,先清掉挡路的,再回头捡宝石。",
  ],
  [
    "冰面上松开方向键还会往前溜一段,提前一点点松手。",
    "幽灵头顶有一个 ⭐,意思是「只有星星打得动」,剑会从它身上穿过去。",
    "滑地板上别贴着怪走,溜过头就撞上去了。",
  ],
  [
    "首领的护甲会来回换颜色:红色只吃剑,蓝色只吃星星,画面顶上会写清楚。",
    "首领冲锋的时候先躲开,它落地那一下会愣一会儿,那时候上去打最划算。",
    "一个人玩的时候记得按 Tab 换人,换成能打得动的那一位。",
  ],
];
...
function buildGuide(): GuideBook {
  ...
  return {
    ...
    general: [
      ...
      "怪头顶的小图标就是答案:⚔️ 找王子,⭐ 找公主。",
      "两个人共用一条心条,替对方挡一下是划算的。",
      "三颗星看的是「清怪、用时、宝石」三样,先求过关,再回头刷成绩。",
    ],
    entries: CHAPTERS.map((ch, ci) => {
      ...
      const entry = {
        ...
        tips: [...(GUIDE_TIPS[ci] ?? [ch.desc]), `本章第 ${from + Math.floor(ch.size / 2)} 关和第 ${to} 关各有一场首领战。`],
      };
      from = to + 1;
      return entry;
    }),
