 ...
 *  - 双人摄像机会把两个人一起夹进画面,拉得太开才退回边上的指路标;
 *  - 无尽城堡塔按层数记成绩;
...
describe("双人摄像机", () => {
  it("只要两人的间距塞得进一屏,谁都不会被挤出画面", async () => {
    const mod = await import("./index");
    const viewW = 560;
    for (const [a, b] of [
      [100, 140],
      ...
      const cam = mod.cameraX([a, b], viewW, 3300);
      expect(mod.onScreen(a, cam, viewW), `${a}/${b} 左边那位`).toBe(true);
      expect(mod.onScreen(b, cam, viewW), `${a}/${b} 右边那位`).toBe(true);
    }
  });

  it("摄像机不会越过关卡两头,也不会为了夹人跑到负数去", async () => {
    const mod = await import("./index");
    expect(mod.cameraX([20, 40], 560, 3300)).toBe(0);
    expect(mod.cameraX([3280, 3290], 560, 3300)).toBe(3300 - 560);
    // 关卡比一屏还短的时候就老老实实贴左边
    expect(mod.cameraX([100, 200], 900, 700)).toBe(0);
  });

  it("实在拉开一屏以上,才轮到边上的指路标顶上", async () => {
    const mod = await import("./index");
    const viewW = 560;
    const cam = mod.cameraX([200, 1400], viewW, 3300);
    expect(mod.onScreen(200, cam, viewW) && mod.onScreen(1400, cam, viewW)).toBe(false);
    // 中点居中,两边掉队的距离一样远,不偏袒谁
    expect(cam).toBeCloseTo((200 + 1400) / 2 - viewW / 2, 5);
  });
});
