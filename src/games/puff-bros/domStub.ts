 ...
 * 排 rAF、`window.setTimeout`、触屏控件的 pointer 事件(带 `pointerId`——
 * 双人同屏「谁抬手松谁的键」那条用例就靠它)、canvas 的 2d 画笔
 * (`arc` / `ellipse` / `setLineDash` 都记下来,机关与形变的用例要数它们)、
...
export interface DrawOp {
  op: "arc" | "ellipse" | "fillText" | "setLineDash" | "gradient" | "fillRect" | "rotate" | "scale";
  args: number[];
  text?: string;
  /** 渐变的色标:天色用例靠它认出「这一章是什么天」 */
  stops?: string[];
  /** 画这一笔时的填充色(机关与气流线的用例靠它认出「这是哪一样东西」) */
  fill?: string;
}
...
export class FakeCtx {
  ...
  rotate(a: number): void {
    this.ops.push({ op: "rotate", args: [a] });
  }
  ...
  ellipse(x: number, y: number, rx: number, ry: number): void {
    this.ops.push({ op: "ellipse", args: [x, y, rx, ry], fill: String(this.fillStyle) });
  }
  roundRect(): void {}
  rect(): void {}
  fill(): void {}
