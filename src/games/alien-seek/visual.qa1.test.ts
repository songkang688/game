/**
 * 1.3 窗口 6 · A 档 · 第 1 轮视觉测试员 · 机器化扫描用例(alien-seek)。
 * 只读断言;问题清单见 docs/qa/1.3-window6-round1-tester.md。
 */
import { readdirSync, readFileSync } from "node:fs";
import { join } from "node:path";
import { describe, expect, it } from "vitest";
import { ALIEN_SPECS, silhouetteKey } from "./visual";
...
describe("窗口6 r1 视觉扫描 · alien-seek", () => {
  it("商标黑名单 0 命中", () => {
  ...
  it("emoji 直出登记:非测试源码含 emoji 的行数只减不增(当前登记 23)", () => {
  ...
  it("六只外星朋友剪影两两不同(路径级差异,不是只换色)", () => {
    expect(ALIEN_SPECS.length).toBe(6);
    const keys = ALIEN_SPECS.map((s) => silhouetteKey(s));
    expect(new Set(keys).size).toBe(6);
  });

  it("星空底 / 星屑彩纸走共享 kit,场景有径向渐变体积", () => {
    const idx = readFileSync(join(DIR, "index.ts"), "utf8");
    expect(idx).toMatch(/art\/kit\/nightsky/);
    expect(idx).toMatch(/art\/kit\/sparkle/);
