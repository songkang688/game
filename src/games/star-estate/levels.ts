 ...
 * 第 8 章残局尤其要紧：坐着等对手自己付破产，地会整批转到鸭梨名下。
 ...
 * 第 8 章残局给对手的起始现金底数。
 *
 * 原来是 150：对手常常第 1 ～ 2 个回合就把自己付破产了，鸭梨还没轮到第二次掷骰，
...
/**
 * 参考解法回放：鸭梨这一格交给「高手」策略自动打，看能不能在限定回合内达标。
...
export function solveLevel(level: number): LevelRun {
  ...
  while (!state.over && state.round <= cfg.rounds && guard < maxSteps) {
    ...
    if (state.players[0].bankrupt) {
      return { win: false, rounds: state.round, netWorth: 0, peak, note: "鸭梨先破产了" };
    }
    if (reached()) {
