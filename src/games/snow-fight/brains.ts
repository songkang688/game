...
export interface AiTuning {
  ...
  dodge: number;
  /**
   * **抢跑**蹲下躲一次要蹲多久(秒)——只管「看见对面举球、赌他要扔」那一种。
   *
   * 球已经飞在半空的那一种反射性蹲避**不读这个字段**,
   * 它按「还有多久砸到我」现算(`me.duck = max(0.1, eta + 0.1)`),躲多久就蹲多久。
   * 也就是说这个字段管的是三档里**用得最少**的那条路。
   *
   * 三档同值(0.35)是量过之后**决定不分**的,不是漏改。第 3 轮实测(12 个种子的对战,
   * 参照方固定跑 `normal` 那一档的调参):
   *
   ...
   * | 0.55 | 8 / 12 | 23 |
   *
   * 初学者那一档**一位数都没动过**。原因是这个字段它够不着:
   * 抢跑蹲避每帧掷一次 `rand() < dodge * dt`,初学者的 `dodge` 只有 0.05/秒,
   ...
   *
   * 还有一条量的时候才发现的坑:现成的对战台把参照方也接在 `normal` 这一档上,
   * 所以**动中间档等于动尺子**。要在这条线上分档,得先给 `dodge` 分档
   * (让低档真的够得着这个字段),并且换一把不跟着动的尺子。那是另一件事。
   */
  duckTime: number;
  /**
   * 算落点的时候把风算进去几成(0..1)。
