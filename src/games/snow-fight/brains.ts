...
export function planThrow(
  ...
  for (const angle of TRY_ANGLES) {
    const charge = solveCharge({ ...from, angle }, target.x, a.wind, target.y);
    if (charge === null) continue;
    const path = flight({ ...from, angle, charge }, { wind: a.wind, groundY: target.y, sampleEvery: 2 });
    let blocked = false;
 ...
 * 认准一个位置走到底就不会了。
 */
export function seekSpot(a: Arena, me: Fighter, target: Vec2): number {
  let best = -1;
  ...
  for (let x = zone.lo; x <= zone.hi + 1e-6; x += SEEK_STEP) {
    ...
    if (!planThrow(a, me, target, hand)) continue;
    if (!planThrow(a, me, target, hand - SEEK_MARGIN)) continue;
    if (!planThrow(a, me, target, hand + SEEK_MARGIN)) continue;
...
export function aiInput(a: Arena, me: Fighter, dt: number, level: AiLevel | null = me.ai): Input12 {
  ...
  if (!me.plan && me.think <= 0) {
    ...
    if (aim) {
      ...
      // 一个都扔不着:照着最想打的那个挪阵地
      me.seek = seekSpot(a, me, wish[wish.length - 1]);
    }
    me.think = tune.react;
  }
  const plan = me.plan;
