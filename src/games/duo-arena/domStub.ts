 ...
 *    (`wrap.innerHTML = ...`),之后全靠 `q(".dua-start")`、
 *    `wrap.querySelectorAll(".dua-modes button")` 这类选择器取节点;
 *    桩要是只把标签剥掉留文字,`mount()` 走两行就取不到东西了。
 *    这里带一个够用的迷你解析器:标签、属性、`class` / `type` / `data-*`、
 *    自闭合标签(`<img>` `<br>`)和文本节点,够本款的模板用。
 *    暂停幕布和虚拟键盘也是二次 `innerHTML` 铺出来再取节点的,一样走它。
 * 2. **`ResizeObserver` 是真桩**。桩把还活着的观察者数出来,
 *    `destroy.test.ts` 才能证明该断的都断了。
 *
 * 本款把 window 上的监听 / 定时器 / 帧循环全记在 `lifecycle.ts` 的账本上,
 * `destroy` 时一句 `life.dispose()` 归零 —— 但 `lifecycle.test.ts` 验的是
 * **那个账本本身**(拿假宿主喂它),没人验过 `index.ts` 真的把每一样都记上了账。
 * 这份桩就是为了补上这一段:真挂一次游戏,数 window 上的线。
 *
...
export class FakeEl {
  ...
  /**
   * 逗号分组 + 空格后代。本款要用到 `.dua-clock .t`、`.dua-modes button`
   * 这样的后代选择器,所以不能只比最后一节 —— 得顺着 parent 往上把前几节对上。
   * 兄弟 / 子代组合符(`>` `+` `~`)本款没用到,不做。
   */
...
export function install(
  ...
  const win = {
    ...
    // 本款的帧循环走的是 `window.requestAnimationFrame`(不是裸的那个),
    // 所以 window 上也得有这一对,不然 `lifecycle` 一开循环就抛。
