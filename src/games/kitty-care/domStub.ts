 ...
 * 极简 DOM 桩（只给本目录的用例用）。
 *
 * 仓库的 vitest 跑在 node 环境、没有 jsdom，也不为了几条用例引新依赖，
 * 所以这里手写舞台真正用到的那几样能力：建元素、挂监听、classList、style、
 * textContent（赋值会清空子节点，和真 DOM 一致）、appendChild / remove、
...
export class StubEl {
  ...
  get textContent(): string {
    return this.children.length > 0 ? this.text + this.children.map((c) => c.textContent).join("") : this.text;
  }

  /** 和真 DOM 一样：赋值会把子节点全部换掉 */
  set textContent(value: string) {
    for (const kid of this.children.splice(0)) kid.parent = null;
...
/** 内存版存档：相册的用例用它，绝不碰真的 localStorage */
...
/** 假的钱包：只记账，方便断言「除了星星什么都没动」 */
export function fakeWallet(start = 0): { getStars: () => number; addStars: (n: number) => number; calls: number[] } {
  let stars = start;
  const calls: number[] = [];
  return {
    calls,
