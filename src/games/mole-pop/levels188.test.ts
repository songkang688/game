...
describe("地鼠嘭嘭 · 关内文案与评星", () => {
  it("提示语按机关拼装，新地洞每一关都有提示", () => {
    expect(levelTips(LEVELS[0])).toBe("视线放在棋盘中间，用余光扫全场！");
  ...
  it("胜负文案按机关分流，失败话术不批评小朋友", () => {
    ...
    expect(winLine(LEVELS[NIGHT[0]], r)).toContain("月光圈");
    expect(winLine(LEVELS[COMBO[0]], r)).toContain("连拍");
    expect(winLine(LEVELS[0], r)).toContain("节奏");
    const fail = { won: false, score: 6, mistakes: 3, timeLeft: 4, bestCombo: 2 };
    expect(loseLine(LEVELS[QUIZ[0]], fail)).toContain("在心里算完得数再落手");
    expect(loseLine(LEVELS[0], { ...fail, mistakes: 0 })).toContain("手指守在棋盘中间");
