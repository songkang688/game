/**
 * 跳跳台 · 1.3 窗口3 第 1 轮视觉验收 · 测试员机器扫描（静态源码断言）。
 *
 * 沉淀给第 2、3 轮复跑的三条底线：
 *  1. 核心资产模块 art.ts 里 fillText 不许直出 emoji（宪法负面清单①：裸 emoji 当核心资产）；
 *  2. art.ts 保有 mixColor 调色与深度雾（本款三阶光影走「预混色阶 + fogAlpha 纵深」路线，
 *     不用 canvas 渐变——哨兵盯这两件，防止 2.5D 纵深退化）；
...
describe("1.3 视觉机器扫描（窗口3 · round1 tester）", () => {
  ...
  it("art.ts 保有 mixColor 调色与深度雾（2.5D 纵深哨兵）", () => {
    const src = read("./art.ts");
    expect(src.includes("mixColor")).toBe(true);
    expect(src.includes("fogAlpha")).toBe(true);
  });
