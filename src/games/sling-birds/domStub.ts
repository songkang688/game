 ...
 *    (`wrap.innerHTML = ...`),之后全靠 `wrap.querySelector(".slb-canvas")`
 *    这类选择器取节点;桩要是只把标签剥掉留文字,`mount()` 走两行就取不到东西了。
 *    关卡格与章节页签也是二次 `innerHTML` 铺出来再取节点的,一样走它。
 * 2. **`ResizeObserver` 是真桩**。本款优先用 ResizeObserver 盯画布尺寸,
 *    桩把还活着的观察者数出来,`destroy.test.ts` 才能证明 `disconnect()` 真的调了。
 *
 * 为什么本款需要这份桩:接手时本款**一条真挂游戏的用例都没有**。
 * 名字里带 `destroy` 的那几条断言(`world.test.ts` / `endless.test.ts` 的
 * `w.destroyed`)说的是**打碎了几块砖**,跟生命周期的 `destroy()` 毫无关系。
 * 而本款在 window 上挂了五个监听(pointermove / pointerup / pointercancel /
 * keydown / resize)、一个 ResizeObserver、一条 `window.setTimeout` 和一条 rAF——
 * 这些从来没有被运行时验证过。
 *
