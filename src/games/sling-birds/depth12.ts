 ...
 *
 * 注：这是 12-A 的第一版解析层，纯几何推导、不跑世界。后来同一格补了一版会真的把整局
 * 跑一遍的实现（`world.ts` 步进 + `sim.ts` 解算 + `materials.ts` / `birds.ts` / `aim.ts` /
 * `endless.ts` 分表），`index.ts` 现在接的是那一版：预测点改成随拉弓长度在 8–12 之间浮动，
 * 可解性也从「够不够得着」升级成「真打一局看清没清台」。本文件的公式与用例继续留着当对照组，
 * 数值口径若要改，两边一起改。
 */
