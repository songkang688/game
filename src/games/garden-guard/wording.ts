 ...
 * 花园守卫 · 说给孩子听的措辞收口。
 *
 * 本作从来不说「血」——头顶那条是**元气条**,怪被打完是没劲了回家。
 * 1.2 新写的文案已经全部按这个口径写,并且有守门用例盯着
 * (`upgrade12.test.ts` 的「1.2 新写的文案一律不说『血』」)。
 *
 * 剩下的几句在**前 99 关**里。那批关卡数据是 1.1 冻结的,`logic.test.ts` 拿
 ...
 *
 * 只用在**玩家看得见**的那一行(关卡提示)。`feature` 字段不渲染,
 * 它只被 `def.feature.includes("BOSS")` 拿去判断是不是 BOSS 关,不用过这道。
 */
