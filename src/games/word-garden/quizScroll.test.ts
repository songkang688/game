/**
 * 守门：钳出滚动条那一档，选项整排得**一进来就露在外面**（窗口5 第 2 轮档A，W5R2-F-A-02 续）。
 *
 ...
 * 一进关**仍旧整颗在裁切线以下**，word-garden 第 41 / 91 / 141 关分别差 39 / 3 / 33px，
 ...
 * 第 41 关实测只要滚 78px（宿主一共能滚 127px），描红卡照旧看得见。
 */
import { describe, expect, it } from "vitest";
import { fitQuizHost, scrollToShowPx } from "./fit";

describe("识字小花园 · 钳出滚动条之后把选字那一排带进眼里", () => {
  describe("scrollToShowPx", () => {
    it("要露的那一段在下面：只滚到刚好露出它的下沿，多一个像素都不滚", () => {
      // 第 41 关实测：选字那一排在宿主内容里占 [221, 418]，可视段 336px，最多能滚 124px
  ...
  describe("fitQuizHost 钳完顺手滚一小段", () => {
    ...
    it("320×568：钳完之后选字那一排的下沿真的进了可视段（原来整颗在裁切线以下）", () => {
