 ...
 *   1. 题库零改动的前提下，第 1–10 章图集配齐——
 *      表情池 + 双胞胎替换对一张不缺；
 *   2. 整关门控：十章全部关卡（含无尽用的 classic 场）贴纸就绪，
 *      绝不出半贴纸半 emoji 混排；
...
import { BOARD_ART_CSS, STICKER_FONT_RATIO, STICKER_READY_THEMES, glyphHTML, sceneStickersReady } from "./boardArt";
import { buildScene } from "./scene12";
import { CHAPTERS, LEVELS, THEME_POOLS } from "./levels";

const PICTO = /\p{Extended_Pictographic}/u;

/** 本轮已配齐图集的主题（第 1–10 章，含双胞胎替换） */
const READY_THEMES = [...STICKER_READY_THEMES];
...
describe("W8R1-04 · 整关门控", () => {
  ...
  it("十章图集都配齐，188 关整关开闸、绝不出半贴纸半 emoji 的混排图", () => {
    let checked = 0;
    for (let level = 0; level < LEVELS.length; level++) {
      expect(sceneStickersReady(buildScene(level, 0)), `第 ${level + 1} 关`).toBe(true);
      checked++;
    }
    expect(checked).toBe(LEVELS.length);
    expect(STICKER_READY_THEMES).toHaveLength(CHAPTERS.length);
  });
});
