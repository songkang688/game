# 1.2 第 29 步 / 共 37 步 · C 档 —— `weiqi-garden`「围子花园」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step29-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 29 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `weiqi-garden` ⚫「围子花园」。
...
必须通读：`src/games/weiqi-garden/` 下 `src/games/weiqi-garden/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
施工 id `weiqi-garden`（别名 weiqi-ink）。提子要有被提起的位移。人机多档+思考延时。

当前 1.1 登记模式：B 接入 19/13/9 路。
本步模式目标：精细化：气与提子动画、打劫、数目、9 路 AI 能玩、多档。
...
## GitHub 结构参考
WGo.js / Sabaki 的气与提子算法（不引运行时）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/weiqi-garden/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
