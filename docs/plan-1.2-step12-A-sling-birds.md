# 1.2 第 12 步 / 共 37 步 · A 档 —— `sling-birds`「弹弹小鸟」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 12 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `sling-birds` 🐦「弹弹小鸟」。
...
必须通读：`src/games/sling-birds/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 关 9 世界。精细化：弹弓拉力手感、轨迹虚线、木/石/玻璃原创材质粉碎过程（不是瞬删）、无商标。

当前 1.1 登记模式：仅 campaign 188。
本步模式目标：无尽沙盒弹射；对战可选轮流。
...
## 独占该游戏目录
只许修改 `src/games/sling-birds/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
