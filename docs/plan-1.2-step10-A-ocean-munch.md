# 1.2 第 10 步 / 共 37 步 · A 档 —— `ocean-munch`「海底大胃王」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step10-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 10 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `ocean-munch` 🐟「海底大胃王」。
...
必须通读：`src/games/ocean-munch/` 下 `logic.ts` `logic.test.ts` `levels188.test.ts` `index.ts` `meta.ts` `guide.ts`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 战役已是 188 关十二海域（洋流、毒藻鱼、共生、深渊压力、海域大王）。`meta.modes` **只有 campaign**，index 无无尽入口。体型差吞噬是侧视大鱼吃小鱼，**不是** `orb-arena` 的 IO。
硬伤：无尽缺失；体型判定在快速擦边时偶发「看起来咬到却没吃到」；窄屏 HUD 体型条/小地图易挡触控。

当前 1.1 登记模式：仅 campaign 188（十二海域）。
本步模式目标：**必须加 endless** 并写入 `meta.modes`。
...
| 闯关 | 保持 188，前 99 参数不动 | 后段可加 1 个新机制（如「潮汐转向」），须可解。 |
| 无尽 | **必须** | 体型随吃增长、地图无限卷轴、比自己大的鱼会追；记录最远距离/最大体型。失败只鼓励。 |
| 对战 | 不做同屏抢鱼（易晕） | 可选「幽灵鱼」追自己纪录。理由写进 meta 注释。 |
体型判定：圆心距与半径差抽常量，擦边容差单测。吃掉用彩纸散开，禁止血/骨描写。
走 `level99.ts` 的保持 **188** 且 `assertTotal(chapters, 188)`；前 99 关生成参数不许改（只许末尾主题已存在的基础上打磨手感）。纯对战不要硬凑 188。

## 2.5D / 3D 决策
保持侧视 **2D** + 最多两层视差。不要 3D 水体，禁止 three.js。
跑酷类在 1.1 第 6 步基础上继续；大鱼吃小鱼加无尽；不要全局 3D。

## 视觉动画
保持侧视 2D + 最多两层视差；不要 3D 水体。吃入口袋缩放 120ms。360px：虚拟摇杆不挡体型条，字号 ≥ 13px。
...
## GitHub 结构参考
开源 side-view 吞噬（半径/体型差判定），不是 IO 俯视
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/ocean-munch/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- 新增用例 ≥ 20：无尽终止、体型判定边界、188 回归、skip 直达。冒烟：战役 Boss、无尽死亡、360px。
...
## 游戏间冲突
不要做成 `orb-arena`。不要改 `snake-snack`。不要引入物理引擎。
