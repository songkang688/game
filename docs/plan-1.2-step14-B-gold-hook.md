# 1.2 第 14 步 / 共 37 步 · B 档 —— `gold-hook`「金矿钩钩」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 14 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `gold-hook` ⛏️「金矿钩钩」。
...
必须通读：`src/games/gold-hook/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已摆钩、重量、商店、188、无尽下探。精细化：摆角手感、钩到物品拉回要有过程、矿洞可用 view25d 做浅透视（可关）。

当前 1.1 登记模式：campaign 188 + endless。
本步模式目标：钩索手感、矿洞浅 2.5D。
...
## 独占该游戏目录
只许修改 `src/games/gold-hook/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
