# 1.3 窗口 8 统筹提示词 —— 窗口 8 · 第 23–26 步（12 款） + 本窗三轮验收（+1） + 做完帮别人

> 把本文件**从下面四行口令起到文末**整段复制，发给窗口 8。不要拆开，不要再套一层摘要。
> 窗口模型选 **claude-fable-5-thinking-xhigh**。仓库基线选 **`game-1.3`**。

请通过 Task 工具派生一或多个云端子代理执行本任务。
指定模型 slug：claude-fable-5-thinking-xhigh。
请在独立功能分支上进行修改，叫 game-1.3-window8。后续都用这个。不要直接修改主分支。
完成后说明修改了哪些文件、测试结果，以及实际使用的模型 slug

【读到这段话的你，就是被派发的**窗口 8 统筹 / 包工头**。上面四行口令是命令**你去转发**，不是命令你自己画。1.2 窗口那种「禁止 Task、自己动手」已经作废。本窗口铁令：

1. **必须用 Task 转发**每一格实现、每一轮验收、以及你去帮别人时的每一格。少派一格、自己动手画、把工人提示词摘要成三句话，都算停工事故。
2. **禁止**自己修改 `src/games/**`、`src/art/**`、`src/ui/**` 里的视觉实现代码。你只许：Task 派工、git 合入/rebase/push、写 `docs/qa/1.3-window8-*` 登记、回复主管。
...
2. `git checkout -B game-1.3-window8 origin/game-1.3`
3. 只写 docs 的「窗口8 · 1.3 视觉统筹计划」commit（可放 `docs/qa/1.3-window8-plan.md` 列本窗格子），**不改游戏代码**。
4. `git push -u origin game-1.3-window8` —— 没有远端分支就派不出 `cloud_base_branch`。
...
| `cloud_base_branch` | 本职工作时填 `game-1.3-window8`；帮窗口 K 时填 `game-1.3-windowK` | 同左 |
| `run_in_background` | `true`（一步三格必须同时派） | 测试员可先单独派；学习优化员与监督修复员等测试员 IDLE 后再并行 |
| `description` | `w8-s<步>-<A\|B\|C>-<id>` 这种短名 | `w8-r<轮>-tester` / `learner` / `fixer` |
...
把下面这段放在 Task `prompt` 最前面，然后**整份**粘上对应 `docs/plan-1.3-step*.md`。粘之前三处替换：所有 `game-1.2-kk` → 目标分支（本职是 `game-1.3-window8`，帮人是 `game-1.3-windowK`）；基线 → `origin/game-1.3`；防套娃段保留为执行者、禁止再 Task。
...
【读到这段话的你，就是被派发的那个云端子代理。上面那句「请通过 Task 派生」是写给派发方看的。你就是执行者，禁止再派生任何云端子代理，必须自己动手把活干完。只推 `game-1.3-window8`，不要改 main / game-1.2 / 1.2-kk，不要 force。本格只改视觉，不改玩法数值与 parentAuth / parentGate。禁止 three.js。】
```

帮窗口 K 时把上面的 `game-1.3-window8` 全部改成 `game-1.3-windowK`。step 正文里的独占文件、测试下限、宪法对照，一字不删。
...
- 本窗口点名：必升：`red-blue-race` 赛道 2.5D。教育三连不许改题目对错。
...
本窗口游戏 id：`red-blue-race red-blue-tap red-blue-tug clock-house math-farm pinyin-train word-garden shape-kingdom find-diff color-fun music-stars kitty-care`
...
替换：`game-1.2-kk` → `game-1.3-window8`；范围改成**仅本窗口游戏** `red-blue-race red-blue-tap red-blue-tug clock-house math-farm pinyin-train word-garden shape-kingdom find-diff color-fun music-stars kitty-care`；报告必须写成：

```
docs/qa/1.3-window8-round1-tester.md
docs/qa/1.3-window8-round1-learner.md
docs/qa/1.3-window8-round1-fixer.md
docs/qa/1.3-window8-round2-tester.md
docs/qa/1.3-window8-round2-learner.md
docs/qa/1.3-window8-round2-fixer.md
docs/qa/1.3-window8-round3-tester.md
docs/qa/1.3-window8-round3-learner.md
docs/qa/1.3-window8-round3-fixer.md
```
...
【你就是执行者，禁止再派生任何云端子代理。只推 `game-1.3-window8`。只验本窗口游戏：red-blue-race red-blue-tap red-blue-tug clock-house math-farm pinyin-train word-garden shape-kingdom find-diff color-fun music-stars kitty-care。报告写到 docs/qa/1.3-window8-round<N>-tester.md。不要碰别人的报告文件。只验视觉，不重开玩法。】
```
...
【你就是执行者，禁止再派生任何云端子代理。只推 `game-1.3-window8`。只评估本窗口游戏：red-blue-race red-blue-tap red-blue-tug clock-house math-farm pinyin-train word-garden shape-kingdom find-diff color-fun music-stars kitty-care。报告写到 docs/qa/1.3-window8-round<N>-learner.md。只学画面密度与角色剪影，不入库任何参考截图，不改绘制代码。】
```
...
【你就是执行者，禁止再派生任何云端子代理。只推 `game-1.3-window8`。只修本窗口游戏视觉：red-blue-race red-blue-tap red-blue-tug clock-house math-farm pinyin-train word-garden shape-kingdom find-diff color-fun music-stars kitty-care。先读 docs/qa/1.3-window8-round<N>-tester.md（以及已有的 learner 报告）。报告写到 docs/qa/1.3-window8-round<N>-fixer.md。一行玩法逻辑都不许动。】
```
...
1. `git fetch origin`（至少 `game-1.3-window8`，本职做完后 8 个窗口分支都 fetch）。
...
本窗口 12 个实现格 + 9 份验收报告都在 `game-1.3-window8`，且 `npm test` / `npm run build` 全绿之后，**不准停机等指令**。你立刻变成机动统筹：
...
4. **仍然必须 Task 转发**，禁止你自己画。工人的 `cloud_base_branch` 和 prompt 里的分支改成 **`game-1.3-windowK`（被帮窗口）**，不要推到你自己的 `game-1.3-window8`。
...
7. 帮完一格就合进 `game-1.3-windowK`，再找下一个最慢窗口。回复主管：「窗口 8 本职已空闲，正在帮窗口 K 的 stepX-Y」。
