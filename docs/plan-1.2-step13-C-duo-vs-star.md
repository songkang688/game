# 1.2 第 13 步 / 共 37 步 · C 档 —— `duo-vs-star`「朵朵大战星星」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 13 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `duo-vs-star` 💥「朵朵大战星星」。
...
必须通读：`src/games/duo-vs-star/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已十二角色弹飞混战、2v2、无尽、188。精细化：击退可读、打出星光而非伤害、平衡补丁、360px 六人混战不糊。

当前 1.1 登记模式：campaign 188 + versus + endless + coop + twoPlayer。
本步模式目标：平衡手感，保持全模式。
...
## GitHub 结构参考
开源 platform-fighter 击退百分比（儿童化击退值）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/duo-vs-star/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
