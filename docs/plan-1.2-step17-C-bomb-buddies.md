# 1.2 第 17 步 / 共 37 步 · C 档 —— `bomb-buddies`「泡泡炸弹人」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step17-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 17 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `bomb-buddies` 🫧「泡泡炸弹人」。
...
必须通读：`src/games/bomb-buddies/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188、对战、无尽收缩。精细化：泡泡膨胀/连锁有过程、软块炸掉有碎裂位移、AI 不自杀。无爆炸伤害描写。

当前 1.1 登记模式：campaign / versus / endless / coop / twoPlayer。
本步模式目标：放泡时机、双人。
...
## GitHub 结构参考
开源 bomber 格子爆炸传播（文案用泡泡）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/bomb-buddies/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
