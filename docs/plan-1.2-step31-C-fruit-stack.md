# 1.2 第 31 步 / 共 37 步 · C 档 —— `fruit-stack`「果果合成」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 31 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `fruit-stack` 🍉「果果合成」。
...
必须通读：`src/games/fruit-stack/` 下 `src/games/fruit-stack/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
**下落与合成必须有过程。** 不是 fruit-catch 接、不是 fruit-slice 切、不是 merge-2048 滑格。

当前 1.1 登记模式：B 接入重力合成。
本步模式目标：精细化：刚体下落过程、同类合成缩放、无尽高度。
...
## 独占该游戏目录
只许修改 `src/games/fruit-stack/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
