# 1.2 第 27 步 / 共 37 步 · B 档 —— `orb-arena`「圆圆大作战」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step27-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 27 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `orb-arena` 🟣「圆圆大作战」。
...
必须通读：`src/games/orb-arena/` 下 `src/games/orb-arena/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
假设 B 第 2–8 步已落地可玩壳（施工 id `orb-arena`，A catalog 别名 `orb-royale`）。本步读现有源码再打磨，不要推倒。不是 ocean-munch。

当前 1.1 登记模式：B 接入：campaign/versus/endless/twoPlayer。
本步模式目标：**接入之后的精细化**：质量手感、分身合并窗口、刺球可读、人机 IO、排行榜折叠。
...
保持 IO 俯视。无尽=一局到只剩自己。闯关 188 若已有则精细化人机波次。吞噬判定常量单测。
...
## GitHub 结构参考
owenashurst/agar.io-clone 的 Canvas 结构；必须本地人机，禁止 Socket
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/orb-arena/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- ≥20 用例；360px 摇杆。若目录不存在：按 B 的 step-10 A 规格完整实现后再做本步精细化，禁止只交壳。
...
## 游戏间冲突
禁止 Socket。不要改 ocean-munch / snake-royale。
