# 1.2 第 33 步 / 共 37 步 · A 档 —— `chess-garden`「国际象棋」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 33 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `chess-garden` ♔「国际象棋」。
...
必须通读：`src/games/chess-garden/` 下 `src/games/chess-garden/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
走子滑移。人机多档+延时。不要和 xiangqi 混规则。

当前 1.1 登记模式：B 接入国际象棋。
本步模式目标：精细化：合法着法、升变、王车易位、多档 AI、残局 188。
...
## 独占该游戏目录
只许修改 `src/games/chess-garden/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
