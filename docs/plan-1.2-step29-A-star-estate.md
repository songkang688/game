# 1.2 第 29 步 / 共 37 步 · A 档 —— `star-estate`「朵星地产」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 29 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `star-estate` 🏦「朵星地产」。
...
必须通读：`src/games/star-estate/` 下 `src/games/star-estate/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
施工 id `star-estate`（别名 star-mogul）。棋子必须沿格走动，不能瞬移。

当前 1.1 登记模式：B 接入掷骰走格。
本步模式目标：精细化：原创地图、机会命运、破产友好、棋子走动过程。
...
## GitHub 结构参考
itaylayzer/Monopoly 走格子状态机（地图卡牌原创，禁 Monopoly 文案）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/star-estate/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
