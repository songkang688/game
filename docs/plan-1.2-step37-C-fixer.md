# 1.2 第 37 步 / 共 37 步 · C 档 —— 监督修复员（第 3 轮）

> 验收三人组第 3 轮。仿 1.1 监督修复员：清阻断/严重。全部在 `game-1.2`。
...
仓库 https://github.com/songkang688/game ，**1.2 第 37 步**角色 **C 监督修复员**（第 3 轮）。独占 `docs/qa/1.2-round3-fixer.md`；修复按 A 报告建议文件，严重度从高到低。
...
## 现状审查
等 A 报告或与 A 同步。读 `docs/qa/1.2-round3-tester.md`（及前轮）。复审 B 的 learner 报告与 diff。

## 通行规则
每修一个：先写能复现的红测试，再修，再全量。禁止 force、禁止改存档语义、禁止商标、禁止新依赖、禁止 three.js。无血无伤文案保持。

## 职责
三轮累计阻断/严重/一般清零；建议级做掉或写明 1.3 再做。终审：商标 0、无外部依赖、存档 key 清单、destroy 抽 15 款×5、分级文案、合并态 SHA 与最终用例总数。
...
## 独占
`docs/qa/1.2-round3-fixer.md` + 为修缺陷改动的源码（以 A 建议路径为准）。不要改 A/B 的报告文件。与 B 同文件冲突时保留更彻底且有测试的一方，并在合并态复验。

## 测试验收
阻断+严重本轮 0 残留。一般级清零；建议级有结论。
全绿。合并态 SHA。
