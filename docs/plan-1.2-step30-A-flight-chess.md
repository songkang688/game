# 1.2 第 30 步 / 共 37 步 · A 档 —— `flight-chess`「飞行棋乐园」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 30 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `flight-chess` ✈️「飞行棋乐园」。
...
必须通读：`src/games/flight-chess/` 下 `src/games/flight-chess/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
棋子必须一格一格飞，禁止瞬移终点。

当前 1.1 登记模式：B 接入四人掷骰。
本步模式目标：精细化：安全区叠子快捷、棋子飞格子过程、人机。
...
## 独占该游戏目录
只许修改 `src/games/flight-chess/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
