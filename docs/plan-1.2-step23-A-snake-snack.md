# 1.2 第 23 步 / 共 37 步 · A 档 —— `snake-snack`「贪吃毛毛虫」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 23 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `snake-snack` 🐛「贪吃毛毛虫」。
...
必须通读：`src/games/snake-snack/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188+无尽花园。**保持格子蛇**，不要做成 snake-royale。精细化：转向缓冲、身体一格一格爬（不要瞬移全身）、双身位等机制。

当前 1.1 登记模式：campaign 188 + endless。
本步模式目标：保持格子蛇；不要做成 IO。
...
## 独占该游戏目录
只许修改 `src/games/snake-snack/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
