# 1.2 第 18 步 / 共 37 步 · B 档 —— `tank-battle`「铁皮坦克大战」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 18 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `tank-battle` 🚜「铁皮坦克大战」。
...
必须通读：`src/games/tank-battle/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已砖/钢/水/草、188、合作对战无尽。精细化：炮弹飞行过程、爆炸改「烟花散开」、双人键位。

当前 1.1 登记模式：campaign / coop / versus / endless。
本步模式目标：弹道、地图。
...
## 独占该游戏目录
只许修改 `src/games/tank-battle/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
