# 1.2 第 31 步 / 共 37 步 · B 档 —— `dot-maze`「豆豆迷宫」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 31 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `dot-maze` 🟡「豆豆迷宫」。
...
必须通读：`src/games/dot-maze/` 下 `src/games/dot-maze/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
角色原创。豆子被吃要有缩放过程。不是 snake-snack。

当前 1.1 登记模式：B 接入迷宫吃豆。
本步模式目标：精细化：转向缓冲、豆子消失过程、人机巡逻、188 迷宫。
...
## 独占该游戏目录
只许修改 `src/games/dot-maze/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
