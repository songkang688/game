# 1.2 第 13 步 / 共 37 步 · A 档 —— `duo-rush`「朵星双人冲刺」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step13-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 13 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `duo-rush` 🏃「朵星双人冲刺」。
...
必须通读：`src/games/duo-rush/` 下 `view25d.ts` `match.ts` `ai.ts` `keys.ts` `logic.ts` `index.ts` 等。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 第 6 步 C 已 2.5D 分屏、无尽、抢金币、幽灵、人机三档、障碍对称单测。本步继续：接共享 view25d、窄屏上下分屏可读、root 的 level 映射到 AI 档或无尽种子。

当前 1.1 登记模式：versus / endless / twoPlayer（无战役表）。
本步模式目标：保持；不要硬凑 188。
...
不做 188 战役（纯竞速）。无尽/对战/幽灵保持。障碍生成必须对称。
...
## GitHub 结构参考
本库 rainbow-run 透视；双人对称障碍生成
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/duo-rush/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
...
## 游戏间冲突
不要改 rainbow-run 的 view3d.ts。键位两套互不抢占。
