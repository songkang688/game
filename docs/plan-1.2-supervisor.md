...
1. **先对齐基线（步 0，派第 1 步之前必须做完）。** 见第三节。本分支已把 `origin/game-1.1`（`0b2df9c`）合进 `game-1.2`：`src/` 与 1.1 一致，库存 55 款，`npm test` 158 文件 / 4456 用例全绿。合入 `game-1.2` 后才许派第 1 步。
...
`game-1.2` 当初是在 1.1 第 6 步中途（`71eb519`）拉出来的，`src/` 曾是 `origin/game-1.1` 的严格子集。
**步 0 已做完**（本分支 merge `origin/game-1.1` @ `0b2df9c`，无冲突）：库存 **55 款**，`src/` 与 1.1 一致，1.2 文档未动。当时缺、现已补上的成品：
...
做法（主管一次做完，不派给子代理；**已经做过，不要再 merge 一次**）：
...
对齐结果（已实跑）：库存 **55 款**（见第五节清单），`npm test` **158 文件 / 4456 用例**全绿，`npm run build` 通过。登记表顶部水位已按这次实跑填写。对齐前 `origin/game-1.2` @ `71eb519` 是 142 文件 / 3918 用例。
