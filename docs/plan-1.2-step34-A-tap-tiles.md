# 1.2 第 34 步 / 共 37 步 · A 档 —— `tap-tiles`「音符下落」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step34-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 34 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `tap-tiles` 🎹「音符下落」。
...
必须通读：`src/games/tap-tiles/` 下 `src/games/tap-tiles/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
**音符必须从上方落下来**，不能在判定线瞬现。不是 music-stars 作曲沙盒。

当前 1.1 登记模式：B 接入下落点按。
本步模式目标：精细化：判定窗 Perfect/Good/Miss、谱面 JSON、下落过程。
...
## 独占该游戏目录
只许修改 `src/games/tap-tiles/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- ≥18 用例含判定窗。目录无则按 B step-16 C 完整实现再精细化。
