# 1.2 第 26 步 / 共 37 步 · B 档 —— `red-blue-tap`「红蓝点点」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 26 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `red-blue-tap` 🎈「红蓝点点」。
...
必须通读：`src/games/red-blue-tap/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188。精细化：判定公平、连击、360px 半屏互不误触。
...
## 独占该游戏目录
只许修改 `src/games/red-blue-tap/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
