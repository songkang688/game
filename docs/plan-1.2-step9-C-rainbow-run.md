# 1.2 第 9 步 / 共 37 步 · C 档 —— `rainbow-run`「彩虹跑跑」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 9 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `rainbow-run` 🌈「彩虹跑跑」。
...
必须通读：`src/games/rainbow-run/` 下 `view3d.ts` `view3d.test.ts` `controls.ts` `endless.ts` `logic.ts` `index.ts` `meta.ts` `guide.ts`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 第 6 步 A **已经做成 2.5D**：`view3d.ts` 纯 Canvas 透视（`scale(depth)=camDepth/(camDepth+depth)`）、三车道、地面网格、视差、远端雾化；`controls.ts` 有土狼时间与输入缓冲；`endless.ts` 程序化路段 + 必过窗口。战役玩法仍跑在平面轨道坐标，透视只发生在绘制。
**本步在此基础上继续，禁止推倒重做，禁止引入 three.js。** 对照后要补的精细化：
1. 接第 1 步 C 的共享 `src/engine/view25d.ts`（若签名不同，做适配层，不要复制第二套相机公式导致两套漂移）。
2. 低端机降画质档位与 30/60fps 手感一致性再加回归。
3. 无尽路段主题多样性（1.1 已有追风棉花云；再加 2 种不改变必过窗口的装饰主题）。
4. 收藏养成（`openCollection`）在跑酷内入口是否好找、满级 +35% 封顶是否仍被遵守。
5. 360px HUD（距离/金币/暂停）不得挡住三键滑动区。
6. root/`?level=N` 直达战役关。

当前 1.1 登记模式：campaign 188 + endless。
本步模式目标：可加 versus/ghost（选做）；不要删无尽。
...
闯关 188 保持；无尽保持并写入 `meta.modes`。对战不做同屏抢车道（那是 `duo-rush`）。可选幽灵跑（对自己最好成绩）。三种失败（撞、掉坑、被追上）文案只鼓励。三键：跳 / 换道 / 下滑；土狼 ~90ms、缓冲 ~120ms 常量单测不许弱化。
走 `level99.ts` 的保持 **188** 且 `assertTotal(chapters, 188)`；前 99 关生成参数不许改（只许末尾主题已存在的基础上打磨手感）。纯对战不要硬凑 188。

## 2.5D / 3D 决策
在 1.1 已有 2.5D 上**继续**，接第 1 步 `src/engine/view25d.ts` 适配。禁止推倒，禁止 three.js。
跑酷类在 1.1 第 6 步基础上继续；大鱼吃小鱼加无尽；不要全局 3D。

## 视觉动画
2.5D 继续加强：远景一层新视差即可，不要上真 3D mesh。障碍/角色仍是 2D 精灵按 scale 缩放。reduced-motion：关闭抖动与强视差，保留换道位移。手机文字：距离与提示 ≥ 14px，安全区留白。
...
## GitHub 结构参考
开源 Canvas 跑酷的分段模板 + 透视投影（只学结构）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/rainbow-run/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- 新增用例 ≥ 18：相机适配、必过窗口 2000 段仍全过、skip 直达、降画质档。冒烟：战役 1/100/188、无尽 60 秒、375 与 1280 两尺寸。
...
## 游戏间冲突
不要改 `duo-rush` 的 `view25d.ts`（那是另一款的文件）。共享引擎 `view25d.ts` 只读，缺能力写进报告让平台步补，本步用适配。不要改 collection 数值封顶。不要全局 3D。
...
完成后回复：角色 C、改了哪些文件、模式与 2.5D 决策、用例增量、测试构建结果、跳关映射、**实际模型 slug**。
