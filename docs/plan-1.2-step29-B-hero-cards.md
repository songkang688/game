# 1.2 第 29 步 / 共 37 步 · B 档 —— `hero-cards`「英杰令」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 29 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `hero-cards` 🎴「英杰令」。
...
必须通读：`src/games/hero-cards/` 下 `src/games/hero-cards/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
施工 id `hero-cards`。禁止三国杀/武将商标。体力叫元气，倒地=休息。

当前 1.1 登记模式：B 接入身份手牌装备。
本步模式目标：精细化：原创武将、距离、技能结算过程、无血。
...
## GitHub 结构参考
开源身份卡牌「身份-手牌-装备-距离」结构，武将名全原创
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/hero-cards/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
