# 1.2 第 27 步 / 共 37 步 · A 档 —— `bowling-lane`「保龄球小馆」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 27 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `bowling-lane` 🎳「保龄球小馆」。
...
必须通读：`src/games/bowling-lane/` 下 `scoring.ts` `logic.ts` `levels.ts` `index.ts` 等。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 @ 8867138 **已有完整实现**（不是规划债）。计分全中 300、188、双人、无尽。精细化：球路 2.5D/伪 3D 透视（Canvas 椭圆球+球道消失点），瓶连锁倒下要有时间线。

当前 1.1 登记模式：campaign 188 / versus / endless / twoPlayer。
本步模式目标：球路透视、计分器保持纯函数。
...
## 2.5D / 3D 决策
可用 Canvas **伪 3D/2.5D**（消失点、椭圆球），禁止 three.js / 真 mesh。
跑酷类在 1.1 第 6 步基础上继续；大鱼吃小鱼加无尽；不要全局 3D。

## 视觉动画
球道可用 view25d；球滚动必须有过程。
...
## 独占该游戏目录
只许修改 `src/games/bowling-lane/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
