# 1.2 第 27 步 / 共 37 步 · C 档 —— `snake-royale`「长蛇争霸」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 27 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `snake-royale` 🐍「长蛇争霸」。
...
必须通读：`src/games/snake-royale/` 下 `src/games/snake-royale/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
施工 id `snake-royale`（别名 snake-clash）。不是 snake-snack 格子蛇。读接入版再打磨。

当前 1.1 登记模式：B 接入 IO 蛇。
本步模式目标：精细化：加速、击杀吃光、转向平滑、本地人机蛇群。
...
## GitHub 结构参考
开源 slither 客户端转向+加速+吃球变长（禁商标/禁联网）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/snake-royale/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- ≥18 用例。目录不存在则按 B step-10 B 完整实现再精细化。
...
## 游戏间冲突
不要改 snake-snack / orb-arena。禁止联网。
