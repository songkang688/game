# 1.2 第 25 步 / 共 37 步 · B 档 —— `color-fun`「涂色小屋」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 25 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `color-fun` 🎨「涂色小屋」。
...
必须通读：`src/games/color-fun/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 指令涂色调色锅。精细化：填色工具手感、安全区、作品可存本机（已有 save 体系）。
...
## 独占该游戏目录
只许修改 `src/games/color-fun/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
