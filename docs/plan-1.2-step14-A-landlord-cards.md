# 1.2 第 14 步 / 共 37 步 · A 档 —— `landlord-cards`「朵朵抢地主」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 14 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `landlord-cards` 🃏「朵朵抢地主」。
...
必须通读：`src/games/landlord-cards/` 下 `logic.ts` `fan.ts` `ai.ts` `sim.ts` `levels.ts` `index.ts` 等。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已完整牌型、三档 AI、188 塔、无尽连胜。精细化：出牌扇形动画（牌要飞出去不是瞬移）、提示「可以出这些」、记牌器开关、困难档配合农民。

当前 1.1 登记模式：campaign 188 地主塔 / versus / endless / twoPlayer。
本步模式目标：牌力提示、三人 AI、出牌动画。
...
## 独占该游戏目录
只许修改 `src/games/landlord-cards/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- ≥20 用例含出牌动画状态机；牌型回归不许弱。
