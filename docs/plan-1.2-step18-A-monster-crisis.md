# 1.2 第 18 步 / 共 37 步 · A 档 —— `monster-crisis`「小怪物危机」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step18-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 18 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `monster-crisis` 👾「小怪物危机」。
...
必须通读：`src/games/monster-crisis/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已守家、路障炮台颜料弹、188。精细化：上色进度条（不要血条）、战场按屏高缩放让方向盘留在屏内、棉花墙不撞脸。

当前 1.1 登记模式：campaign 188 / endless / coop / versus。
本步模式目标：无血文案、波次、手机守家。
...
## 视觉动画
2D。360px 方向盘必须在舞台内（1.1 已修过，加回归冒烟）。
...
## 独占该游戏目录
只许修改 `src/games/monster-crisis/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
