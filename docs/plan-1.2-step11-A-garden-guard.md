# 1.2 第 11 步 / 共 37 步 · A 档 —— `garden-guard`「花园守卫」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step11-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 11 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `garden-guard` 🌼「花园守卫」。
...
必须通读：`src/games/garden-guard/` 下 目录内 logic/levels/index/meta/guide 及测试。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 关十三主题、多种塔。精细化：种植/升级手感、主题建模（花瓣炮等原创）、手机 HUD 不挡路径、跳关直达、攻略不泄最优布局。

当前 1.1 登记模式：仅 campaign 188 塔防。
本步模式目标：可选无尽波次；对战不做（塔防不同屏抢怪）。
建议 `meta.platform`：`both`（第 1 步缺省 both，本步改准）。
...
闯关保持 188。无尽：波次无限、花种有限。不要 PVP。塔的射程/冷却纯函数单测。
...
## 视觉动画
2D 俯视花园，不要 3D 地形。种植有下落/扎根动画（短）。360px 建造栏可滚动。
...
## GitHub 结构参考
开源 tower-defense 的 range/cooldown 结构
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/garden-guard/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- ≥18 用例；冒烟第 1/188 关与建造。
...
## 游戏间冲突
不要改 `sprout-defense`（那是植物路线守家，本款是塔）。不要写僵尸/商标植物名。
