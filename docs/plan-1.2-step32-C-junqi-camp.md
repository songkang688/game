# 1.2 第 32 步 / 共 37 步 · C 档 —— `junqi-camp`「军旗对决」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 32 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `junqi-camp` 🎖️「军旗对决」。
...
必须通读：`src/games/junqi-camp/` 下 `src/games/junqi-camp/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
碰撞比大小用「收编」不是消灭。棋子翻开有过程。

当前 1.1 登记模式：B 接入军棋。
本步模式目标：精细化：暗子揭示动画、铁路行棋、人机多档、无血。
...
## 独占该游戏目录
只许修改 `src/games/junqi-camp/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
