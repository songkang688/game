# 1.2 第 19 步 / 共 37 步 · C 档 —— `fishing-star`「钓鱼小达人」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 19 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `fishing-star` 🎣「钓鱼小达人」。
...
必须通读：`src/games/fishing-star/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已抛竿水层 25 鱼 188 无尽。精细化：张力小游戏手感、鱼入护过程、图鉴收集接 collection 或本游戏图鉴 key。
...
## 独占该游戏目录
只许修改 `src/games/fishing-star/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
