# 1.2 第 17 步 / 共 37 步 · B 档 —— `alien-seek`「寻找外星朋友」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 17 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `alien-seek` 🛸「寻找外星朋友」。
...
必须通读：`src/games/alien-seek/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 手绘场景、推理、无尽、双人抢答。精细化：热区 ≥44px、窄屏两图、线索不泄答案坐标。

当前 1.1 登记模式：campaign 188 / endless / versus / twoPlayer。
本步模式目标：找物热区、主题。
...
## 独占该游戏目录
只许修改 `src/games/alien-seek/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
