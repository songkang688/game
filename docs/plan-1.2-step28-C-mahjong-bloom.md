# 1.2 第 28 步 / 共 37 步 · C 档 —— `mahjong-bloom`「花开麻将」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 28 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `mahjong-bloom` 🀄「花开麻将」。
...
必须通读：`src/games/mahjong-bloom/` 下 `src/games/mahjong-bloom/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
施工 id `mahjong-bloom`。不是连连看翻牌。牌打出必须有飞牌过程。

当前 1.1 登记模式：B 接入国标简化。
本步模式目标：精细化：吃碰杠胡动画、81 番种表校验、三档 AI、牌面飞入。
...
## GitHub 结构参考
i711net/mahjong 四人桌结构（不是 solitaire）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/mahjong-bloom/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
