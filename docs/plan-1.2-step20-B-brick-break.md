# 1.2 第 20 步 / 共 37 步 · B 档 —— `brick-break`「碰碰砖块」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 20 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `brick-break` 🧱「碰碰砖块」。
...
必须通读：`src/games/brick-break/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188。精细化：反弹手感、砖碎裂过程（不能瞬删）、关卡砖型。
...
## 独占该游戏目录
只许修改 `src/games/brick-break/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
