# 1.2 第 10 步 / 共 37 步 · C 档 —— `fight-king`「朵星格斗王」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 10 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `fight-king` 🥋「朵星格斗王」。
...
必须通读：`src/games/fight-king/` 下 `frames.ts` `rules.ts` `engine.ts` `ai.ts` `levels.ts` `controls.ts` `index.ts` `meta.ts` `guide.ts` 及测试。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已有八段原创角色、三段帧、格挡/破防/投技/受身、连段上限 6、能量超必、五种模式、188 格斗塔。本步是**手感与可读性精细化**，禁止把角色/招式表扩写成 `combo-clash`（那是另一套 6 名格斗家 + 以帧计取消窗口的新 id）。

当前 1.1 登记模式：campaign 188 格斗塔 / versus / endless / twoPlayer / 训练。
本步模式目标：加深手感，不要做成 `combo-clash`。
...
保持五种模式。升级：输入缓冲再校准、受身窗口可视化（训练模式）、塔关 AI 增益可读、无尽连胜的对手表不要只加数值。人机至少保持三档，可再加一档「会反抓投技」。无血：元气条、星星飞溅、眩晕转圈。
...
## 视觉动画
保持 2D 卡通。命中顿帧/轻震；reduced-motion 全关抖动。360px：虚拟摇杆 + 轻/重键不挡角色。不要 3D 舞台。
...
## GitHub 结构参考
既有本目录帧数据拆法；开源 2D 格斗取消表思路
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/fight-king/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- 新增用例 ≥ 20：缓冲窗口、受身、塔 188 回归、destroy。冒烟：双人键位、塔 1/188、训练帧显示。
...
## 游戏间冲突
禁止复用或改 `combo-clash` 招式。不要改 `duo-vs-star`。不要写「KO」「HP」。
