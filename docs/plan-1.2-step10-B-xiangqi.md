# 1.2 第 10 步 / 共 37 步 · B 档 —— `xiangqi`「朵朵星星象棋」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 10 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `xiangqi` 🐘「朵朵星星象棋」。
...
必须通读：`src/games/xiangqi/` 下 `logic.ts` `logic.test.ts` `index.ts` `meta.ts` `guide.ts`（可新建 `ai.ts` `puzzles.ts` 及测试）。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已有完整棋规纯函数（车马炮兵将士相、九宫、河界、将军、将帅对面）和双人同屏 + **单一**「棋灵象」人机，将军提示、悔棋一档。**没有残局战役、没有多档 AI。**
`dark-chess` 是 B 档新游戏（揭棋），不要和本目录搅在一起。本步只升级已有 `xiangqi`。

当前 1.1 登记模式：versus + twoPlayer（只有棋灵象一档人机，无闯关）。
本步模式目标：残局闯关 188 + 人机多档 + 连胜；不新建第二个象棋 id。
...
| 残局学堂 | 必须 | 188 道可解残局（≥8 章：将杀一步、双车错、马后炮、困毙、官和、限时快棋…）。每题解唯一或解集明确，全部可解单测。走 level99 或自建地图但接同一套星/跳关。 |
| 自由对战 | 必须 | 人机 **≥5 档**（菜鸟/入门/普通/高手/地狱）：菜鸟会漏将、地狱 3–4 层 + 思考延时 300–800ms。固定 seed 档位碾压断言。 |
| 连胜 | 必须（当无尽） | 升档连胜。 |
| 揭棋/暗棋 | **不做** | 那是 `dark-chess`。 |
规则：长将/长捉判和或禁着（选一种写清）；困毙；60 回自然限着可和。将军高亮路径；应将点必须可点。落子确认（手机开）。
...
## 视觉动画
保持 2D 交叉点棋盘，红黑原创棋字（不要用商标字体）。走子滑移 160ms。360px：9×10 用 hit slop，坐标字可关。不要 3D 棋子挡点。
...
## GitHub 结构参考
开源中国象棋合法着法/将军检测（只学规则结构）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/xiangqi/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。

## 测试验收
- `npm test` 与 `npm run build` 全绿；只增不删不调弱。
- 新增用例 ≥ 40：合法着法回归、残局 188 可解、五档胜率、长将、skip 直达。冒烟：双人一局、残局 1/188、地狱思考动画。
...
## 游戏间冲突
禁止新建 `xiangqi-2` 或改 `dark-chess`。不要把棋子画成写实兵器。空格不要抢壳层暂停。
