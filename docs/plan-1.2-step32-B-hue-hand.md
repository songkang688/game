# 1.2 第 32 步 / 共 37 步 · B 档 —— `hue-hand`「花色接龙」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step32-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 32 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `hue-hand` 🌈「花色接龙」。
...
必须通读：`src/games/hue-hand/` 下 `src/games/hue-hand/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
不是 klondike 接龙，是色彩出牌。A catalog 的 klondike-cards 未在 B 21 款里，不要另开。

当前 1.1 登记模式：B 接入色彩手牌。
本步模式目标：精细化：出牌规则、+2 链、人机、出牌飞入。
...
## 独占该游戏目录
只许修改 `src/games/hue-hand/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
