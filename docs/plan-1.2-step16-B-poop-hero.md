# 1.2 第 16 步 / 共 37 步 · B 档 —— `poop-hero`「便便超人」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 16 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `poop-hero` 🦸「便便超人」。
...
必须通读：`src/games/poop-hero/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188、清洁马拉松、双人。精细化：题材保持干净可爱不恶心、跳跃手感、臭臭怪变花要有变身过程。

当前 1.1 登记模式：campaign 188 / endless / coop。
本步模式目标：分级干净、手感。
...
## 独占该游戏目录
只许修改 `src/games/poop-hero/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
