# 1.2 第 33 步 / 共 37 步 · C 档 —— `hop-pads`「跳跳台」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step33-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 33 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `hop-pads` ⭕「跳跳台」。
...
必须通读：`src/games/hop-pads/` 下 `src/games/hop-pads/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
A 别名 lily-hop。蓄力有过程，小人跳起抛物线必做。

当前 1.1 登记模式：B 接入蓄力跳跃。
本步模式目标：精细化：蓄力条、落点预览、无尽台子、浅 2.5D。
...
## 独占该游戏目录
只许修改 `src/games/hop-pads/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
