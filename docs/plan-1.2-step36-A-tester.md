# 1.2 第 36 步 / 共 37 步 · A 档 —— 测试员（第 2 轮）

> 验收三人组第 2 轮 / 共 3 轮。仿 1.1 第 14 步。全部在 `game-1.2`。三轮样本不重复（本轮回归点名款除外）。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 第 36 步**角色 **A 测试员**（第 2 轮）。独占报告 `docs/qa/1.2-round2-tester.md` 与你新增的 `*.test.ts`（**只补测试，不改玩法**）。
...
`clock-house`、`math-farm`、`pinyin-train`、`word-garden`、`shape-kingdom`、`find-diff`、`color-fun`、`music-stars`、`kitty-care`、`lianliankan`、`memory-cards`、`mole-pop`、`puzzle-tiles`、`snake-snack`、`brick-break`、`merge-2048`、`mine-garden`、`sudoku-petal`、`dot-maze`、`fruit-stack`、`pool-stars`、`hue-hand`、`junqi-camp`、`chess-garden`
   每款：首页进入 → 真实胜负（赢一次、输一次）→ 退出再进；有战役则 1 / 100 / 188；有对战/无尽则各试；双人键位抽测互不抢占。
3. 必须先读第 1 轮三份报告，遗留一般/建议本轮处理或关闭。
专项：
- 换样本 24 款（上列），与第 1 轮尽量不重复。
- 难度曲线：抽 8 款记录 90–110 与 170–188 通关耗时/失败次数。
- 手感：跑酷/格斗/平台（若第 1 轮已深测则改测合成物理/台球/跳跳台）。
- 教育性：学习 6 款攻略不泄题。
- 竞态：快速切游戏/暂停恢复 20 次。
- 第 1 轮点名 5 款做**回归**（只确认修复，不占新样本名额）。
4. 产出 `docs/qa/1.2-round2-tester.md`。

## 2.5D / 视觉 / 手机 / 跳关
按专项检查，不要自己改渲染或密码门。

## 独占
`docs/qa/1.2-round2-tester.md` + 新建/补强的 `*.test.ts`。禁止改玩法 `src/games/**` 非测试文件，禁止改 B/C 的报告路径 `*-learner.md` `*-fixer.md`。

## 测试验收
全绿；只增测试。走查中发现的缺陷用红测试锁住（能锁的锁）。

## 游戏间冲突
不要修别人的代码。建议责任文件写清，让 C 修。不要和 B 抢同一 `*.test.ts`（B 若落地测试，你用不同文件名，例如 `*.audit.round2.test.ts`）。
