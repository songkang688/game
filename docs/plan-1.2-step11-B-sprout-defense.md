# 1.2 第 11 步 / 共 37 步 · B 档 —— `sprout-defense`「绿芽保卫战」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 11 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `sprout-defense` 🌱「绿芽保卫战」。
...
必须通读：`src/games/sprout-defense/` 下 目录内全部玩法文件。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 关、七种植物九种虫。精细化：路线/波次可读、种植动画、可选浅 2.5D 花田透视（用 view25d，可关）。

当前 1.1 登记模式：仅 campaign 188。
本步模式目标：无尽虫潮可选；对战不做。
...
保持 188。无尽虫潮。克制表纯函数。不要与 garden-guard 共用数据文件。
...
## 视觉动画
默认 2D；若接 2.5D 只做地面透视，角色仍 2D。禁止 three.js。
...
## GitHub 结构参考
开源 lane-defense 冷却与克制表
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/sprout-defense/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
...
## 游戏间冲突
不要改 garden-guard。不要写官方植物/昆虫商标名。
