# 1.2 第 11 步 / 共 37 步 · C 档 —— `fruit-slice`「水果切切乐」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 11 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `fruit-slice` 🍑「水果切切乐」。
...
必须通读：`src/games/fruit-slice/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 回合 + 无尽。精细化：刀光拖尾、连刀判定、炸弹果安全文案（「苦果」不要爆炸伤害描写）、安全区。
...
## 视觉动画
2D；刀光粒子 reduced-motion 可关但切片位移保留短过程。360px 全屏可划。
...
## GitHub 结构参考
开源 swipe-slice 的多边形切割思路（自制几何，不引库）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/fruit-slice/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
...
## 游戏间冲突
不是 fruit-catch / fruit-stack。不要忍者商标。
