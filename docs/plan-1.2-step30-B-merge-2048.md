# 1.2 第 30 步 / 共 37 步 · B 档 —— `merge-2048`「星星合成」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 30 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `merge-2048` 🔢「星星合成」。
...
必须通读：`src/games/merge-2048/` 下 `src/games/merge-2048/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
**格子滑动必须有过程**，合并有缩放。与 fruit-stack 不同（那是物理球）。

当前 1.1 登记模式：B 接入滑向合并。
本步模式目标：精细化：滑移过程（不能瞬合）、闯关目标板、无尽最高分。
...
## GitHub 结构参考
gabrielecirulli/2048 滑向合并（禁商标）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/merge-2048/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
