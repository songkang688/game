# 1.2 第 16 步 / 共 37 步 · A 档 —— `box-hamster`「推箱小仓鼠」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 16 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `box-hamster` 🐹「推箱小仓鼠」。
...
必须通读：`src/games/box-hamster/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 可解、冰面、传送门、双鼠。精细化：撤销栈、死锁温和提示、最优步三星、动画一格一格推（箱子要滑过去）。
...
保持 188 可解硬性。对战不做。无尽：随机小残局（生成器保证有解）。
...
## 独占该游戏目录
只许修改 `src/games/box-hamster/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
