...
> **第 11 轮更新（主干解冻）**。基线：`origin/game-1.3 = 0cb201b3`（r19 收口合入）：**N-108 = puzzle-tiles 无尽画廊拼块滚不到**（🔧 B 唯一新开必修，`.l99-host` hidden 第 4 次复发）；**N-109 = root 密码门矮横屏折线下**（🟡 已书面降级，盒内滚得到）；**N-100 扩面 6→17 款**（坐实 tab 折行款全库系统病，仍只修 level99.ts 一处锚定）；收口降级台账落地（N-10 残余/N-109/red-blue-race 芯片）。**新军规「可达性判据」**见 §6.9。862b r21「跳过 108…116」被主干打破——主干已用 108/109，其拟名 N-117…124 数字不受影响。**引用一律用主干号（水位 N-109）**；重复在途项一律「只等先合版」（§3.0 封存清单）。
...
**「只等先合版」封存清单（重复在途，谁都不许再写新份；合入后只跑回归留数字）**：

- **N-105 红灯**：**六路**——`46b26d1e` / `18c63e9d` / `9124d56b` / 4e78 / b255 / c337 `5063f8f1`
- **N-99 数独盘面**：三路——5c27 被派 / 4e78 内滚 / b255 模式条（后者另一伤面可共存，合并核对选择器）
- **N-100 误滚卷顶**：两路——5c27 被派 / b255（b255 已实现「当前关整格可见不再 scrollIntoView」=**部分修复**；残余「页签折多行/当前关在首屏外仍 center 滚」由拟名 N-117 页签收纳收尾）
- **N-12 / N-10 / N-3**：双开——`1856baa6` / 9efa `e32540ba`
- **N-55 / N-81**：三开+追改——`1c93b984`（fixed 钉底）/ 3c67（两列一行，又追 `45b75fab` 桌面档兜底+字号回 16px）/ 9efa（雪战十二键）
- **C-8 天空钳高**：9efa 已认领
- **模式键 44px**（=拟名 N-121/迭代U-2）：9ad5 `51f8fcc6` 已修 4 款，先合则只剩 balloon-pop/duo-rush 复测
...
| 9efa `e32540ba` 内「N-90 判定线」 | N-90 | 与主干 N-90（tap-tiles 已结案）**同号异伤**，合入须改号 |
...
- [ ] ~~N-12 / N-10 / N-3~~ **在途已双开勿三开**（`1856baa6` 与 9efa `e32540ba` 各修一版）——先合版生效；合入后按旧数字回归（N-3 还欠 1024×768/1180×820 平板档数字，迭代U-1 口径）
...
- [ ] ~~N-55~~ **在途已三开勿四开**（`1c93b984`/3c67/9efa 三方案）——先合版生效，合入后回归 412 内数字
- [ ] ~~C-8~~ **在途已修**（9efa `e32540ba` 天空钳显示高）——先合版生效；合入后确认 `logic.ts` SKY_H=420 未被动
...
## 七、第 11 轮给 #1/#2 的可勾选执行清单（主干解冻轮）

任务号与独占以主干 r18/r19 playbook 为权威；**动手前先读 §3.0 封存清单 + §6.9 可达性判据**。**禁双开（零容忍）**：N-105 红灯（六路）、N-99 数独（三路）、N-100 误滚（两路+b255 部分已修）、N-12/N-10/N-3（双开，N-10 残余已降级）、N-55/N-81（三开+追改）、C-8（9efa 认领）、ba/cs 地图（e75a 认领）——全部「只等先合版」，落地后只跑回归留数字。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `0cb201b3`），按序执行：

- [ ] **先交卷压箱货**：红灯 `46b26d1e`（六路取先合）→「root 管理员行」`980945e8` →「密码门」`4970d8ef`（**交卷改号 N-109**，主干已定名且降级——此单从必修转「余力项已完成」）→「挑拣车厢」`c2a21b4c`（勿用 N-94，注明别名待统编）
- [ ] **N-100 验收面升级**：修的仍是 `level99.ts` 一处锚定，但**验收从 6 款扩到 17 款**（清单在主干 `trio-r19-learn-notes.md` §四）——写成跑全库 tab 折行款的循环脚本，别逐款打补丁
- [ ] **封存项零动作**：N-99 先对账 4e78/b255；动 styles.css 前先 fetch 9ad5 两单（`7a4f732e`/`1bdf3287`）
- [ ] **空白项**：N-97 math-farm 深关选项 416 → 拟名 N-123 首页平板 hero（首卡 557→≤500，只加档）→ 幽灵热区抽查 quiz99 系 3 款（`elementFromPoint` 断言）→ **壳层静态断言**（§6.9 第 3 条，根治 `.l99-host` 四次复发，强烈建议本轮做）
- [ ] 拟名 N-117/N-118/N-120 壳层族：等 862b r21 合入定名后再动，只读不写
- [ ] 结案落账：N-60/61/62/N-90/N-91 回归数字（禁再修）；N-11/N-46/N-56 先并号（迭代U-13/U-19 疑同伤）；**降级台账 3 项不占必修位**（N-10 残余/N-109/red-blue 芯片，升回条件=「滚不到」）
- [ ] U-101 竖屏首扫（b255 报告先合则以其 390 账为底）；U-2 横屏左右安全区（`styles.css:261`）
- [ ] 交卷：水位 ≥ 1193/19489 且 N-105 守门 5 红转绿；🟡/🔧 按 §6.9 分开列；报告进当轮 tester-A 文件

**#2（B 休闲对战）**——基于 `game-1.3` 最新（≥ `0cb201b3`），按序执行：

- [ ] **先交卷压箱货**（三单）：brave-path `a6ed4010` → N-81/N-55 `1c93b984`（三开取先合）→ N-12/N-10/N-3 `1856baa6`（双开取先合；N-10 残余已降级，只取不冲突部分；合入后补 N-3 平板档 1024×768/1180×820 数字）
- [ ] **封存项零动作**：红灯/N-55/C-8/N-12/10/3/ba-cs 地图勿写新份
- [ ] **空白项第一档（主干定名必修，按序）**：**N-108 puzzle-tiles**（主干唯一新开必修——钳拼块边长或 `.pz-board` 内滚+热区尾款五件套，勿改 seed；=迭代U-17，引用用 N-108）→ N-98 hue-hand → N-95 xiangqi 设置屏（阻断级）→ N-94 dvs 选人 → N-101 dvs 赛中 14 键（同目录连修）→ **N-107 fruit-stack 六键**（最重）→ N-96 bomb-buddies → N-106 monster-crisis 摇杆 → N-103 ice-fire → N-102 bumper → N-104 landlord h33
- [ ] **空白项第二档（647a 九伤，先并号再修）**：迭代U-13 bowling CTA（对账 N-11）→ U-14 flight-chess 掷骰 → U-18 match-stars 盘（P0）；U-16/N-108/U-18/U-21 与 C-5 同族走「网格高度反推钳」一个配方连修（每款独立提交+独立回归数字）；U-19 sky-squad 对账 N-46/N-56
- [ ] 拟名 N-121 模式键复测（9ad5 先合则只剩 balloon-pop/duo-rush）；切底伤逢款顺手补 768–1024 档（拟名 N-124）
- [ ] U-201 竖屏首扫（4e78 舞台竖滚先合则六款切底账重量）；U-5 选关格抽查（e75a 先合只剩 brave-path）；U-6 reduced-motion
- [ ] 交卷：水位只增不减；每条两档数字（切底伤加 1024×768）；🟡/🔧 按 §6.9 分开列；报告进当轮 tester-B 文件

**两位共通**：§3.0 + §6.9 先读，碰封存项 = 打回；**并号优先于开新**；**初见出屏先 scrollIntoView 再定性**（🟡 滚得到 ≠ 🔧 滚不到）；门类流程回归走真 UI；贴线族先查壳层预算；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；**新伤只记「文件+数字」勿自编号**——主干水位 N-109，在途拟名已排到 N-124，开 N-110…116 会三方撞车；撞车取先合版。
