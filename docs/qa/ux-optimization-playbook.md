...
> **第 18 轮更新**。基线：`origin/game-1.3 = 206d0522`（主干六轮稳）。**重灾警报**：N-99 已**五路**、N-100 已**四路**、N-97 已**双开**（b255 `f8455b6e` 与 9a87 撞满）——这三号绝对零动作，只等先合版。测试水位新基准 **1202/19513**（1cd5 r21A 交卷补记 `87870e12`）。号池经 1cd5 r25/r26 推进到 **N-137**，下一空号 **N-138**；r26 台账已做一批并号裁决（`.dvs-back`→N-94/N-101、`.ld-back`→N-104、`.shr-back`→N-134、`.rbe-back`→N-133）——**返回钮类新伤先查这批再说**。勿开 N-110…116。
...
- **N-99 数独盘面**：**五路**——5c27 被派 / 4e78 内滚 / b255 模式条 / 9a87 `b7677155` 盘可滚+守门 / **b255 `f8455b6e` 矮横屏直修**——先合版生效，其余全弃
...
| 1cd5 `87870e12`+r25/r26 笔记（第 18 轮新收） | r21A 交卷补记——**vitest 水位 1202/19513（新基准）**；r25 开 N-135…137、r26 从 N-138 起并做返回钮并号裁决（dvs→N-94/101、ld→N-104、shr→N-134、rbe→N-133、四款无高度返回→N-135） | 在途；号池 N-137，下一空号 N-138；**返回钮类新伤先查 r26 并号表** |
...
## 七、第 18 轮给 #1/#2 的可勾选执行清单（两位仍在 r2）
...
- [ ] **r2 空白项（A 真正动手位，按序）**：**N-128 壳层静态守门**（1cd5 立号，R-9 阶梯③，根治五次复发的系统病）→ 幽灵热区 R-4 全仓粗筛（`rg "inset:0"` + elementFromPoint 断言）→ 拟名 N-123 首页平板 hero（首卡 557→≤500，只加档）→ **PT 报告遗留**：gold-hook/junqi-camp/dark-chess 平板置顶留白（壳层 opt-in 类方案，勿全局 safe center）、duo-arena 菜单 43% 宽
- [ ] **N-127（=N-124 扩面）A 份**：R-3 container query 底座（`.l99-stage` container-type）——20 款清单在 1cd5 r22 笔记；逐款救济档归 B
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-137**，下一空号 N-138——但新伤仍只记「文件+数字」交学习员统编）；**返回钮类伤先查 1cd5 r26 并号表**；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
