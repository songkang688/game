...
> **第 5 轮更新**。基线：`origin/game-1.3 = 8b23ab11`（主干连续两轮未动，在途分支大量堆积——本轮重点是 §3.0 号池消歧表）。
> **定位**：与 `trio-rN-learn-notes/playbook` 的逐案抽验并行，本文是「模式层」清单。**本波任务号以主干 `trio-r18-playbook.md` 为权威**（含独占文件表），本文叠加 U-x 模式层与竖屏首扫。
> **编号纪律**：本文条目一律用 **U-x**，不占 N 号。⚠️ **N-92…N-102 已出现三套并存定义**（主干 vs 两个在途学习员分支），动手前必看 **§3.0 消歧表**；新伤一律只记「文件+数字」待统编。
> ⚠️ **U 号撞车**：并行学习员 `docs/ux-iteration-playbook.md`（分支 `ux-learning-playbook-647a`，在途）的 U-1…U-6 与本文定义不同。消歧：先合版为准；引用对方写「**迭代U-x**」，本文写「U-x」。#1（A）领 U-101…，#2（B）领 U-201…。
> **对账纪律**：动手前 `git log --oneline -20` + 查对应 `*.test.ts`；同伤在途多方案的（N-81/N-55/N-82/红灯，见 §3.0）**先合版生效，其余弃**。
> **视口口径（父监督红线）**：主测 = 手机竖屏 **390×844**（必须能划到底）+ 平板横屏 **915×412**（地图/按钮不裁切）；切底伤加测 **1024×768**（数字可引迭代 playbook）。管理员密码 `kangkang`，默认 1 小时、可永久，密码不落盘；量 root 态可直接种 `yiduo-yixing.root.v1 = {"expiresAt":253370764800000,"mode":"permanent"}`。
...
### 3.0 N-92…N-102 号池消歧表（第 5 轮，动手前必读）

**权威序**：主干 `8b23ab11`（N-94…N-97 = duo-vs-star 开打 / xiangqi 设置屏 / bomb-buddies 画布 / math-farm 深关；N-98 = hue-hand 三键；N-99 = sudoku-petal 盘面）。在途分支合入时撞号者**必须改号**（先例：c8a3d154 撞号改 N-94…N-97）。

| 在途出处 | 其自编号 | 内容 | 消歧结论 |
| --- | --- | --- | --- |
| #1 A `980945e8`/`4970d8ef` | N-92 / N-93 | root 管理员行收一行 / 密码门矮横屏收进一屏 | 新伤，占用被弃用号——先合则定名，报告须写明「（root 管理员行）/（密码门）」 |
| 学习员 c337 `f9cd2577` | N-93 | duo-vs-star 选人开打 439–488 | **= 主干 N-94 同伤**，勿双开 |
| 学习员 c337 | N-97 | xiangqi 自由对战 701–755 | **= 主干 N-95 同伤**，勿双开 |
| 学习员 c337 | N-99 | combo/mahjong 红灯 | 已被三路修（`46b26d1e` 先发），勿再开 |
| 学习员 c337 | N-92 / N-94 / N-95 / N-96 / N-98 | **真新伤**（下详，已并入 §3.1，标〔c337〕待统编） | 内容收录，号待改 |
| 学习员 862b `f28dcad2` | N-98…N-102 | 章节页签堆高 / 地图密度 136px / 划不动排查 / 地图视觉 / mapColumns 容器宽 | **全部撞主干 N-98/N-99**，号全作废待改；内容收录 §3.1/§四，标〔862b〕 |
| #2 B `1c93b984` vs r17B `3c67` | N-81 / N-55 | B=fixed 钉视口底；3c67=两列 / 操作牌一行 | **同伤双方案在途**——先合版生效，后到者 rebase 弃自己那份 |
| r17B `1711193a` vs r18 笔记 | N-82 | 3c67=盘左抬头右 | r18 笔记口径「进局实测未绿则修」，若 3c67 先合则只回归 |
...
- [ ] 〔c337〕duo-vs-star **赛中**触屏 14 键 400–746 全线下（915；canvas 122–302 IN；≠ 选人屏开打=主干 N-94。触屏两人没法打，B 重）
- [ ] 〔c337〕bumper-cars 画布仅 **140×140**@915（390 竖屏同局 331×331）+ 模式芯片 h=34/32 <44；1024×768 刹车键 741–785 底切 17（只放大显示画布，勿改碰撞世界尺寸）
- [ ] 〔c337〕ice-fire-forest 闯关 L1 画布底切 **59**（canvas 232–471，`.l99-stage` 滚 220/394；只钳显示高或改 fit，勿动关卡逻辑网格）
- [ ] 〔c337〕landlord-cards「◀ 回选关」h=**33** <44（轻；叫分/暂停行已绿勿动）
...
- [ ] 〔c337〕**l99「开始冒险 ▶」与工具行被卷出视口顶**（915 进场即负 top：word-garden −154/−104、ice-fire −104、xiangqi −50、landlord −52、bumper −39；root×pinyin「直达」−54——scrollIntoView 聚焦当前关把 `.l99-view` 卷过头；390/1024 全 IN。修法：聚焦后校正 `.l99-view` scrollTop 或改 `block:"nearest"`；N-63 的 stage 归零勿回退）
...
| 学习员 c337 `f9cd2577`（在途） | l99 continue 负 top 族（word-garden −154 等）；dvs 赛中 14 键 400–746；bumper 画布 140；冰火切 59；landlord 回选关 h33 | 真新伤已并入 §3.1（标〔c337〕），号待统编 |
| 学习员 862b `f28dcad2`（在途） | 章节页签 4 行 ≈200px；`136px` 常数过时；`.l99-view` 无 touch-action；地图视觉；mapColumns 容器宽 | 号全撞主干待改，内容已并入 §3.1/§五（标〔862b〕） |

## 七、第 5 轮给 #1/#2 的可勾选执行清单

任务号与独占文件以主干 `trio-r18-playbook.md` 为权威；**动手前先读 §3.0 消歧表**。视口红线：**390×844 划到底 + 915×412 不裁切**，每条两档留数字；切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `8b23ab11`），按序执行：

- [ ] **先交卷压箱货**：`46b26d1e`（红灯，三路撞车先合版生效）+「N-92 root 管理员行」`980945e8` +「N-93 密码门」`4970d8ef`——报告写明号名防混淆；越晚合越容易被别人撞
- [ ] **N-99 sudoku-petal**（主干号，A 最重）：盘底两行 391–504 滚不到 → 钳格或 `.sp-wrap` 内滚；390 勿回退；配小测试
- [ ] 〔c337〕l99 continue/工具行负 top 族（word-garden −154 等 6 款 + root×pinyin −54）：聚焦后校正 `.l99-view` scrollTop 或 `block:"nearest"`；N-63 stage 归零勿回退——**一处壳修多款受益，优先级仅次于 N-99**
- [ ] N-97 math-farm 深关选项 416；迭代U-4 首页平板 hero（首卡 557→≤500，只加档）
- [ ] 〔862b〕壳层份滚动排查：`.l99-view` 补 `touch-action:pan-y`（一行 + 断言）；章节页签堆高与 136px 常数（改 `.l99-tab` DOM 必联动 `rootUnlock.ts:108`）
- [ ] A 面结案落账：N-60/61/62/N-90/N-91 回归数字（禁再修）；N-11/N-46/N-56 复测仍开则修
- [ ] U-101 竖屏首扫（只补缺口）；U-2 横屏左右安全区（`styles.css:261`）
- [ ] 交卷：水位 ≥ 1193/19489 且 5 守门红转绿；报告进当轮 tester-A 文件

**#2（B 休闲对战）**——基于 `game-1.3` 最新（≥ `8b23ab11`），按序执行：

- [ ] **先交卷压箱货**：brave-path 大厅 `a6ed4010` + N-81/N-55 fixed 方案 `1c93b984`（与 r17B `3c67` 双开——先合版生效，若 3c67 先合则 rebase 弃自己份）
- [ ] **N-98 hue-hand**（主干号）三键 422–466：sticky→fixed（N-75 配方；390 竖屏 713–757 IN 勿回退）
- [ ] N-95 xiangqi 设置屏 713 滚不到（阻断级；c337 的「N-97」同伤勿双开）；N-94 开打 451（c337 的「N-93」同伤）；N-96 bomb-buddies 画布 63
- [ ] 〔c337〕duo-vs-star **赛中** 14 键 400–746（≠ 选人屏，B 重）；〔c337〕ice-fire 闯关画布切 59；〔c337〕bumper-cars 画布 140 过小+芯片 34/32；〔c337〕landlord 回选关 h33
- [ ] **N-3 star-estate 四视口一次修**（§3.1 修法；500 档守门勿动）；N-12 pool-stars 两档修
- [ ] 迭代U-2 模式键 44px 三款复测补 min-height；C-8 只钳 `.blp-sky` 显示高
- [ ] 〔862b〕各游戏份滚动排查：扫「设置/选人屏 overflow:hidden 且无矮屏媒体」（N-95 同构族）
- [ ] U-201 竖屏首扫（8 款）；U-5 选关格 44px 抽查（l99 主图免量）；U-6 reduced-motion 扫描
- [ ] 交卷：水位只增不减；每条两档数字（切底伤加 1024×768）；报告进当轮 tester-B 文件

**两位共通**：§3.0 消歧表先读；贴线族先查壳层预算（N-89 教训）；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；新伤只记「文件+数字」待统编；撞车取先合版。
