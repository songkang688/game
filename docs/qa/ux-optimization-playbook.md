...
> **第 10 轮更新（继续收敛）**。基线：`origin/game-1.3 = 6982da7e`（主干连续三轮未动）。号池现况：862b r21 自废 N-108…116、拟名 N-117…N-124 在途；647a r3 又出**迭代U-13…U-21 九伤**（全 B 面，含 3 个 P0）+「网格高度反推钳」同族配方；红灯 N-105 六路在途。**引用一律用主干号（水位 N-107）**，新伤勿自编号；重复在途项一律「只等先合版」（§3.0 封存清单）。
...
| 在途出处 | 其自编号 | 消歧结论 |
| --- | --- | --- |
| #1 A `980945e8`/`4970d8ef`/`c2a21b4c` | N-92 / N-93 / N-94 | 前两个占弃用号可先合定名；**「N-94」撞主干（dvs 选人）**，合入须改号或写明「（挑拣车厢）」 |
| 862b r19 `c33650c1` / r20 `5bc04d4a` | N-103…107 / N-108…116 | **两批号全作废**；r21 `50abe7a6` 已自裁：N-108…116 整段永久跳过，内容归并主干号或迁 **N-117…N-124（在途拟名）**：N-117 页签堆高、N-118 地图密度+136px 常数、N-119 地图观感、N-120 触摸滚动口径（部分被 4e78 覆盖）、N-121 模式键<44（=迭代U-2）、N-122 duo-rush 竖屏 CTA（=迭代U-3）、N-123 首页 hero 平板（=迭代U-4）、**N-124 平板断点空洞：33 款 l99 游戏只有 500 档救济**（=本文 G-4 实锤成清单）；mapColumns 容器宽降级不开号 |
...
| 学习优化-C 647a（r3 `b6c139e5`） | U-7…U-21 | 番号撞车持续，引用冠「迭代U-x」。r3 新九伤**全 B 面**：U-13 bowling CTA（P0，**疑与 N-11 同伤，动手先并号**）、U-14 flight-chess 掷骰（P0）、U-18 match-stars 消除盘（P0）、U-15 hero-cards、U-16 memory 网格+模式键 36、U-17 puzzle-tiles、U-19 sky-squad D-pad 38–42（疑与 N-46/N-56 同伤）、U-20 prince-princess 平板、U-21 mine-garden（自注若 r14 有号则并号）。其 U-6/U-8 已自并入 N-105/C-5。**同族配方「网格高度反推钳」**：aspect-ratio:1 网格在矮横屏按宽膨胀顶穿舞台——C-5/U-16/U-17/U-18/U-21 五款一个配方连修 |
...
- [ ] **N-100** l99「开始冒险 ▶」与工具行被卷出视口顶（915 进场即负 top：word-garden −154/−104、ice-fire −104、xiangqi −50、landlord −52、bumper −39；root×pinyin「直达」−54——只咬章节 tab 折 2 行以上的款；390/1024 全 IN。⚠️ b255 已部分修复（§3.0 封存清单），残余面由拟名 N-117 页签收纳收尾；N-63 的 stage 归零勿回退）
...
| 学习优化-C r3 `b6c139e5`（第 10 轮新收） | 迭代U-13…U-21 九伤（bowling CTA / flight-chess 掷骰 / match-stars 盘三个 P0）+ 网格高度反推钳配方；U-6/U-8 自并 N-105/C-5 | 在途；全 B 面，U-13 疑= N-11、U-19 疑= N-46/N-56，动手先并号 |

## 七、第 10 轮给 #1/#2 的可勾选执行清单（继续收敛）

任务号与独占以主干 r18 playbook（主文+两附录）为权威；**动手前先读 §3.0 封存清单**。**禁双开（零容忍）**：N-105 红灯（六路）、N-99 数独（三路）、N-100 误滚（两路+b255 部分已修）、N-12/N-10/N-3（双开）、N-55/N-81（三开+追改）、C-8（9efa 认领）、ba/cs 地图（e75a 认领）——全部「只等先合版」，落地后只跑回归留数字。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（四单）：红灯 `46b26d1e`（六路取先合，他路先合则弃）→「root 管理员行」`980945e8` →「密码门」`4970d8ef` →「挑拣车厢」`c2a21b4c`（**勿用 N-94，注明别名待统编**）
- [ ] **封存项零动作**：N-99/N-100 先 `git log` 对账 4e78/b255——已合只回归留数字，未合也不新写；**动 styles.css 前先 fetch 9ad5 两单**（胜负弹层 `7a4f732e` + 全局 touch-action `1bdf3287`）防冲突
- [ ] **空白项**（A 真正动手位）：N-97 math-farm 深关选项 416 → 拟名 N-123 首页平板 hero（首卡 557→≤500，只加档）→ 幽灵热区抽查 quiz99 系 3 款（`elementFromPoint` 断言；4e78 先合则只回归）
- [ ] 拟名 N-117/N-118/N-120 壳层族（页签收纳/136px 常数/pan-y）：**等 862b r21 合入定名后再动**，本轮只读证据不写码
- [ ] A 面结案落账：N-60/61/62/N-90/N-91 回归数字（禁再修）；**N-11 bowling 先并号**（迭代U-13 疑同伤，见 §3.0，谁修谁在报告写明并号）；N-46/N-56 同理（迭代U-19 疑同伤）
- [ ] U-101 竖屏首扫（只补缺口，b255 报告 `acde7247` 先合则以其 390 账为底）；U-2 横屏左右安全区（`styles.css:261`）
- [ ] 交卷：水位 ≥ 1193/19489 且 N-105 守门 5 红转绿；报告进当轮 tester-A 文件

**#2（B 休闲对战）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（三单）：brave-path `a6ed4010` → N-81/N-55 `1c93b984`（三开取先合）→ N-12/N-10/N-3 `1856baa6`（双开取先合；合入后补 N-3 平板档 1024×768/1180×820 数字）
- [ ] **封存项零动作**：红灯/N-55/C-8/N-12/10/3/ba-cs 地图全部勿写新份，把手转空白项
- [ ] **空白项第一档（主干定名 P0，按序）**：N-98 hue-hand 422–466（sticky→fixed）→ N-95 xiangqi 设置屏 713（阻断级）→ N-94 dvs 选人 451 → N-101 dvs 赛中 14 键（与 N-94 同目录连修）→ **N-107 fruit-stack 双人六键**（最重）→ N-96 bomb-buddies 画布 63 → N-106 monster-crisis 摇杆（修键别动技能卡）→ N-103 ice-fire 切 59 → N-102 bumper 画布 140 → N-104 landlord h33
- [ ] **空白项第二档（647a r3 九伤，动手先并号再修）**：迭代U-13 bowling CTA（**先对账 N-11**）→ 迭代U-14 flight-chess 掷骰 → 迭代U-18 match-stars 盘（此三为 P0）；U-16/U-17/U-18/U-21 与 C-5 五款同族，走「**网格高度反推钳**」一个配方连修（每款独立提交+独立回归数字）；迭代U-19 sky-squad **先对账 N-46/N-56**
- [ ] 拟名 N-121 模式键复测（9ad5 先合则只剩 balloon-pop/duo-rush）；修切底伤逢款顺手补 768–1024 高度档（拟名 N-124 断点空洞，勿专开大扫）
- [ ] U-201 竖屏首扫（8 款；4e78 `e9995575` 舞台竖滚先合则军棋/台球/地产/飞行棋/果堆/英雄卡切底账全部重量）；U-5 选关格抽查（e75a 先合只剩 brave-path）；U-6 reduced-motion 扫描
- [ ] 交卷：水位只增不减；每条两档数字（切底伤加 1024×768）；报告进当轮 tester-B 文件

**两位共通**：§3.0 封存清单先读，碰封存项 = 打回；**并号优先于开新**（bowling/sky-squad/mine-garden 类旧号先 `rg docs/qa` 对账）；贴线族先查壳层预算（N-89 教训）；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；**新伤只记「文件+数字」勿自编号**——在途拟名已排到 N-124，统编交学习员/监督；撞车取先合版。
