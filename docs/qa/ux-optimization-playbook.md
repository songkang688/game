...
> **第 25 轮更新**。基线：`origin/game-1.3 = 17356717`（水位 1205/19520）。本轮三事：① **#1（e75a）`bce2e3a3`**——N-97 合后残余（矮屏选项钉宿主可视底）+**PT-5 math-farm 布景包含块钉舞台**（stagebar 不再被盖 = **幽灵热区族 R-4 首个学习款实修**）；② **5f46 交平板横屏全量体检报告**（9 项修复、4 视口回归、19488 用例全绿——其基线较旧，主干口径仍 1205/19520）；③ 号池 r33 从 **N-159** 续，**裁决：match-stars 格子并入 N-127**（=迭代U-18 定名），新豁免三条（迷宫 `--cell` div、弹弓 coach-dot 装饰、碰碰车 `.bc-knob` 整颗摇杆即热区）。小热区族仍归 1cd5；勿开 N-110…116。
...
## 七、第 25 轮给 #1/#2 的可勾选执行清单（小热区族让给 1cd5）
...
**#1（A 壳+学习，r2 已交 N-100 R-1 版）**——基于 `game-1.3` 最新（≥ `206d0522`），按序执行：

- [ ] **交卷收尾（PR #99 已胜出）**：PT 四单+守门 8 例保持可合；`07ad9de1` **rebase 到 ≥ `17356717` 后拆单**——N-100 源码部分弃（主干已修），**只保留 l99host 守门 106 行断言差集与 17 款验收数字**单独提
- [ ] **已销账只回归**：N-99/N-97/N-100/N-109 各留两档回归数字（N-100 跑 17 款清单）；N-117/118/120（1cd5 实修）、顶栏挤扁（b255/4e78）仍只对账，碰 = 打回
...
- [ ] 交卷：水位只增不减（新基准 **1205/19520**）+ 红灯守门绿；🟡/🔧 分开列；报告进当轮文件
...
- [ ] **647a 九伤剩余（先对账 5f46/1cd5 再动）**：保龄球/飞行棋/记忆翻牌 5f46 已认领勿双开；U-19→N-153、U-20→N-154 属 `--k` 族（1cd5 节奏快，认领前必 fetch）；**U-18 match-stars 已裁并 N-127**（断点空洞族口径修，勿单独开）；仍空白的：迭代U-15 hero-cards、U-21 mine-garden（先并号 r14）；网格同族走「高度反推钳」配方
- [ ] **小热区/返回钮/模式胶囊类一律让给 1cd5**（N-121…N-152 两大单已包）——#2 只做滚动/CTA 出屏/画布裁切类大伤，撞款白干
- [ ] ~~N-125 / N-126~~ **1cd5-B `06dee785` 已实修勿双开**（§3.0）——只对账，合入后回归；sticky 类新修仍先过 §八 R-9 判据
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-158**，下一空号 N-159——但新伤仍只记「文件+数字」交学习员统编）；**返回钮/小热区类伤先查 1cd5 r26–r32 并号表与豁免录**（hh-back/se-deed/pcp-btn/poop 摇杆/xq-seg 已豁免或钉回归勿报）；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
