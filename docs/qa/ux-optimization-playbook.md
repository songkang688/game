...
> **第 23 轮更新**。基线：`origin/game-1.3 = 17356717`（N-99/97/100/109 已销账只回归；水位 1205/19520）。号池经 1cd5 r30/r31 推进到 **N-155**，下一空号 **N-156**。**647a 两个空白伤正式定名**：N-153 = sky-squad 方向键 `--k`<44（≈迭代U-19/N-46/56 族并号）、N-154 = prince-princess 双人 D-pad 格子 34–42（≈迭代U-20 族；`.pcp-key` 钉了 44 但格子 `--k` 更矮，两套尺码打架——修法学 poop 改列数/让画布，**勿撤 min-height**）；N-155 = `--k` 数字地板静态巡检（A）。豁免录新增 poop 摇杆（`padMetrics` ≥44，`MIN_HOT_DUO=34` 是注释遗物不开号）。勿开 N-110…116。
...
| 1cd5 r30/r31 笔记（第 23 轮新收） | r30 开 N-150…152（brave-path/pzt 工具+`.pcp-act`/veil-btn）；r31 开 N-153…155（**sky-squad `--k`≈迭代U-19 族、prince-princess D-pad≈迭代U-20 族、`--k` 地板巡检 A**）+poop 豁免 | 在途；号池 N-155，下一空号 N-156；647a 剩余空白伤又收编两个 |
...
- [ ] **647a 九伤剩余（先对账 5f46/1cd5 再动）**：保龄球/飞行棋/记忆翻牌 5f46 已认领勿双开；**U-19→已定名 N-153、U-20→已定名 N-154**（引用用 N 号；认领前 fetch 1cd5 tester 分支对账；N-154 修法勿撤 min-height）；仍空白的：迭代U-18 match-stars 盘（P0）、U-15 hero-cards、U-21 mine-garden（先并号 r14）；网格同族走「高度反推钳」配方
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-155**，下一空号 N-156——但新伤仍只记「文件+数字」交学习员统编）；**返回钮/小热区类伤先查 1cd5 r26–r31 并号表与豁免录**（十余类全有主；hh-back/se-deed/pcp-btn/poop 摇杆已豁免勿报）；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
