...
> **第 7 轮更新**。基线：`origin/game-1.3 = 6982da7e`（又一波撞号两让：云学习员 r9 复测独有项定名 **N-106/N-107**，重叠项并号 N-103/104/105 作独立复证；**N 号水位 N-107**）。
...
**权威序（主干 `6982da7e`）**：N-94…N-97 = duo-vs-star 选人开打 / xiangqi 设置屏 / bomb-buddies 画布 / math-farm 深关；N-98 = hue-hand 三键；N-99 = sudoku-petal 盘面；N-100 = l99 进场卷顶（A）；N-101 = dvs 赛中键柱（B 重）；N-102 = bumper-cars 画布+触区（B）；N-103 = ice-fire 画布切 59（B，含深关补证）；N-104 = landlord 回选关 h33（B 轻）；N-105 = 主干红灯（B 第零优先）；**N-106 = monster-crisis 双人摇杆/甩弹切底 ~50（B）；N-107 = fruit-stack 双人六键 522–566 全线下（B 重）**。撞号者合入时必须改号（先例三次：c8a3d154、e140c26b、6982da7e）。
...
| 学习员 862b `c33650c1`（r19 笔记） | N-103…N-107 | 选关地图 UX 族（页签堆高/136px/pan-y/视觉/mapColumns） | **N-103…N-107 现已全被主干占用**，其号全作废，预期落 N-108+；内容已收 §3.1/§五（标〔862b〕） |
| #2 B `1856baa6`（第 7 轮新收） | N-12 / N-10 / N-3 | 台球双栏+台面钳余高 / 围棋整盘进屏 / 地产模式屏真预算+座位收行 | **三旧号在途已修**——§3.1 对应项转「先合版生效，勿双开」 |
...
- [ ] ~~N-12 / N-10 / N-3~~ **在途已修**（#2 `1856baa6`：台球双栏+钳台面 / 围棋整盘 / 地产真预算+座位收行）——先合版生效勿双开；合入后按 §3.1 旧数字回归（N-3 还欠 1024×768/1180×820 平板档数字，迭代U-1 口径）
...
## 七、第 7 轮给 #1/#2 的可勾选执行清单

任务号与独占以主干 r18 playbook（主文+两附录）为权威；**动手前先读 §3.0**。**禁双开**：N-105 红灯（五路在途）、N-99 数独（三路）、N-100 误滚（两路）、N-12/N-10/N-3（`1856baa6` 在途）、N-81/N-55（双方案）。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（四单）：红灯 `46b26d1e`（他路先合则弃）→「root 管理员行」`980945e8` →「密码门」`4970d8ef` →「挑拣车厢」`c2a21b4c`（**改号 N-108 起或注明别名**，勿用 N-94）
- [ ] **N-99 数独 / N-100 误滚**：先 `git log` 对账 4e78/b255 是否已合——已合只回归留数字，未合才动手（§3.1 修法）
- [ ] N-97 math-farm 深关选项 416；迭代U-4 首页平板 hero（首卡 557→≤500，只加档）
- [ ] 〔862b〕壳层滚动排查：`.l99-view` 补 `touch-action:pan-y`；章节页签堆高与 136px 常数（联动 `rootUnlock.ts:108`；862b r19 号已全作废但内容有效，对账后做）
...
**#2（B 休闲对战）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（三单）：brave-path `a6ed4010` → N-81/N-55 fixed `1c93b984`（与 r17B 双开先合版生效）→ **N-12/N-10/N-3 `1856baa6`**（合入后补 N-3 平板档 1024×768/1180×820 数字）
- [ ] **N-105 红灯**：勿写第六份，先合版落地后跑守门确认转绿
- [ ] **滚动/CTA 出屏族（按序）**：N-98 hue-hand 422–466（sticky→fixed）→ N-95 xiangqi 设置屏 713（阻断级）→ N-94 dvs 选人 451 → N-101 dvs 赛中 14 键 400–746（与 N-94 同目录连修）→ **N-107 fruit-stack 双人六键 522–566**（本批最重新伤）→ N-96 bomb-buddies 画布 63
- [ ] **N-106** monster-crisis 双人摇杆/甩弹切底 ~50（修键别动技能卡）；N-103 ice-fire 画布切 59；N-102 bumper 画布 140（芯片若 9ad5 先合只回归）；N-104 landlord h33
...
**两位共通**：§3.0 先读；贴线族先查壳层预算（N-89 教训）；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；新伤只记「文件+数字」待统编（下一个空号 **N-108**）；撞车取先合版。
