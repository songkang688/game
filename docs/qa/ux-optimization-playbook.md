...
> **第 9 轮更新（收敛版）**。基线：`origin/game-1.3 = 6982da7e`（主干仍未动）。号池好消息：862b 的 r21 **自废 N-108…N-116**（整段永久跳过）、归并回主干 N-100…N-105，新伤改从 **N-117…N-124** 续编（在途拟名，见 §3.0）。坏消息：红灯 N-105 冒出**第六路**在途修（c337 `5063f8f1`）。**引用一律用主干号（水位 N-107）**，新伤勿自编号；重复在途项一律「只等先合版」。
...
| 独有新伤单（无撞车，先合即定）| — | 9ad5 `7a4f732e` l99 胜负弹层+320 弹窗；4e78 `7007612f` 幽灵热区（§一）、`e9995575` l99 舞台竖滚（军棋/台球/地产/飞行棋/果堆/英雄卡 stage-wrap 剪死）；b255 `ae99317d` 顶栏 43→67+收藏册安全区 |
| 学习优化-C 647a | U-7…U-12 | 与本文 U-x 番号撞车持续；引用冠「迭代U-x」，先合版拿正名 |
...
## 七、第 9 轮给 #1/#2 的可勾选执行清单（收敛轮）

任务号与独占以主干 r18 playbook（主文+两附录）为权威；**动手前先读 §3.0 封存清单**。**禁双开（本轮零容忍）**：N-105 红灯（**六路**在途）、N-99 数独（三路）、N-100 误滚（两路+b255 部分已修）、N-12/N-10/N-3（双开）、N-55/N-81（三开+追改）、C-8（9efa 认领）——这些全部「只等先合版」，落地后只跑回归留数字。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。
...
- [ ] **封存项零动作**：N-99/N-100 一律先 `git log` 对账 4e78/b255 是否已合——已合只回归留数字，未合也**不新写**（在途方案已够多），把手转空白项
- [ ] **空白项**（本轮 A 真正动手位）：N-97 math-farm 深关选项 416 → 拟名 N-123 首页平板 hero（首卡 557→≤500，只加档）→ 幽灵热区抽查 quiz99 系 3 款（`elementFromPoint` 断言；4e78 先合则只回归）
- [ ] 拟名 N-117/N-118/N-120 壳层族（页签收纳/136px 常数/pan-y）：**等 862b r21 合入定名后再动**，本轮只许读证据不许写码
...
- [ ] **封存项零动作**：红灯/N-55/C-8/N-12/10/3 全部勿写新份，把手转空白项
- [ ] **空白项**（本轮 B 真正动手位，按序）：N-98 hue-hand 422–466（sticky→fixed）→ N-95 xiangqi 设置屏 713（阻断级）→ N-94 dvs 选人 451 → N-101 dvs 赛中 14 键（与 N-94 同目录连修）→ **N-107 fruit-stack 双人六键**（最重）→ N-96 bomb-buddies 画布 63 → N-106 monster-crisis 摇杆（修键别动技能卡）→ N-103 ice-fire 切 59 → N-102 bumper 画布 140 → N-104 landlord h33
- [ ] 拟名 N-121 模式键复测（9ad5 先合则只剩 balloon-pop/duo-rush）；修切底伤逢款顺手补 768–1024 高度档（拟名 N-124 断点空洞，勿专开大扫）
- [ ] 各游戏份滚动排查：扫「设置/选人屏 overflow:hidden 且无矮屏媒体」（N-95 同构族）
- [ ] U-201 竖屏首扫（8 款；4e78 `e9995575` 舞台竖滚若先合，军棋/台球/地产/飞行棋/果堆/英雄卡切底账全部重量）；U-5 选关格 44px 抽查；U-6 reduced-motion 扫描
- [ ] 交卷：水位只增不减；每条两档数字（切底伤加 1024×768）；报告进当轮 tester-B 文件

**两位共通**：§3.0 封存清单先读，碰封存项 = 打回；贴线族先查壳层预算（N-89 教训）；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；**新伤只记「文件+数字」勿自编号**——在途拟名已排到 N-124（862b r21），统编交学习员/监督；撞车取先合版。
