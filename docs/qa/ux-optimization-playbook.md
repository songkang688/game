...
> **第 12 轮更新（大合并落地）**。基线：`origin/game-1.3 = 206d0522`。本拍主干四连合：**N-12/N-10/N-3/N-55/N-81 + 红灯 16px 守门已合**（`76d20324`，9fc4 版胜出）；**C-8 与 N-90 tap-tiles 判定线摘合**（`0f6c0820`，9efa 版摘选）；**迭代 playbook 正式合入**（PR #83——迭代U-x 从此是主干文档，引用不再加「若合入」）；监督对账 PR #82。→ §3.0 封存清单**大清仓**：上述各号全部转「已合只回归」，其余五路红灯/双开单全弃。水位仍 **N-109**；在途拟名 N-117…124；勿开 N-110…116。新大户：**5f46 平板横屏 8 连单**（认领迭代U-13/U-14/U-16+断点空洞族，bubble-aim 与 e75a **双开**）。
...
- **bubble-aim 地图放宽**：**新双开**——e75a `8b41214a`（竖屏解锁滚动+ba/cs 放宽）vs 5f46 `f72b937f`（680/820px 6 列）——先合版生效
...
## 七、第 12 轮给 #1/#2 的可勾选执行清单（大合并后重排）

任务号与独占以主干 r18/r19 playbook + 迭代 playbook（PR #83 已是主干文档）为权威；**动手前先读 §3.0（已合清仓+封存）与 §6.9 可达性判据**。**已合只回归**：红灯 N-105、N-12/N-10/N-3、N-55/N-81、C-8、N-90 判定线——再写新份 = 打回。**仍封存**：N-99 数独（三路）、N-100 误滚（两路）、bubble-aim（e75a vs 5f46 双开）、l99 竖滚（4e78 vs 5f46 双开）、模式键+胜负弹层（9ad5 交卷待合）。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `206d0522`），按序执行：
...
- [ ] 拟名 N-117/N-118 壳层族：**5f46 已带地图 680→820（N-118 密度族先实修）**——对账后只做页签收纳（N-117）不重做密度；N-120 pan-y 对账 4e78/5f46 竖滚安全网
...
**#2（B 休闲对战，已在 r2）**——基于 `game-1.3` 最新（≥ `206d0522`），按序执行：

- [ ] **r1 交卷收尾**：模式键/胜负弹层/touch-action/红灯对齐四单（`fd4b0f8d`…`56893a7b`）保持可合状态，冲突时 rebase 勿force；合入后迭代U-2 只剩 balloon-pop/duo-rush 复测
...
- [ ] **647a 九伤剩余（先对账 5f46 再动）**：保龄球/飞行棋/记忆翻牌 5f46 已认领——**勿双开**，只等先合版；仍空白的：迭代U-18 match-stars 盘（P0）、U-15 hero-cards、U-20 prince-princess、U-21 mine-garden（先并号 r14）、U-19 sky-squad（对账 N-46/N-56）；网格同族走「高度反推钳」配方
- [ ] 已合回归落账：N-12/N-10/N-3（N-3 补平板档数字）、N-55/N-81、C-8（SKY_H 未动）各留两档数字；9efa 残余单 `20fcbec4` 先合则台球/地产回归以其为底
- [ ] U-201 竖屏首扫；U-5 选关格抽查（bubble-aim 双开只等先合，剩 brave-path/candy-swing）；U-6 reduced-motion
