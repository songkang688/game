...
> **第 20 轮更新**。基线：`origin/game-1.3 = 206d0522`（主干八轮稳）。认领潮再进一格：**1cd5-B `06dee785` 一单修 N-125…N-139 热区族**（sprout-defense/shoot-range 等 14 文件，自注「错开 A、不回退 N-121 族」）——**#2 被派的 N-125/N-126 转「对账勿双开」**。号池经 r27/r28 推进到 **N-143**，下一空号 **N-144**；并号表再扩：`.ld-btn` 42→N-141、`.fk-mode`→N-142、仓鼠 `.bh-btn`→N-47（PR #79 在途）、`.bba-swap`→N-132（kit 到 44 即绿）。水位基准 1202/19513。勿开 N-110…116。
...
| 1cd5-B `06dee785`+r27/r28 笔记（第 20 轮新收） | 一单修 N-125…N-139 热区 44（14 文件，错开 A 不回退 N-121 族）；r27 开 N-141…143、r28 从 N-144 起；并号再扩（ld-btn→141、fk-mode→142、bh-btn→N-47、bba-swap→N-132） | 在途；**#2 的 N-125/N-126 转对账**；号池 N-143，下一空号 N-144 |
...
## 七、第 20 轮给 #1/#2 的可勾选执行清单（两位仍在 r2）
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-143**，下一空号 N-144——但新伤仍只记「文件+数字」交学习员统编）；**返回钮/小热区类伤先查 1cd5 r26–r28 并号表**（dvs/ld/shr/rbe/mp/sp-key/ld-btn/fk-mode/bh-btn/bba-swap 全有主）；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
