...
> **第 22 轮更新（主干解冻）**。基线：`origin/game-1.3 = 17356717`——**PR #99（9a87 版）合入：N-99/N-97/N-100/N-109 四号销账**，全库新水位 **1205/19520 全绿**。撞车结算：N-100 五路中 9a87「锚定 sticky+钳滚距」胜出；e75a 的 R-1 版 `07ad9de1` 落选，但其 **17 款验收数字与 l99host 守门 106 行断言差集仍值得单独提**（先合版没带这套守门）；b255 `f8455b6e`、4e78 内滚、1cd5 `fbc6655b` 的 N-99 部分全弃。号池 N-149 在途、下一空号 N-150；勿开 N-110…116。豁免录（hh-back/se-deed/pcp-btn）沿用。
...
| 5f46 `51eece04`（第 22 轮新收） | 寻找外星朋友补显式行模板（修 `grid-row:1/-1` 跨行失效），找物关侧栏不再裁缩放/望远镜 | 在途；grid 隐式行陷阱可入 §八 R-2 附注 |
...
## 七、第 22 轮给 #1/#2 的可勾选执行清单（N-97/99/100/109 已销账）
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-149**，下一空号 N-150——但新伤仍只记「文件+数字」交学习员统编）；**返回钮/小热区类伤先查 1cd5 r26–r30 并号表与豁免录**（十余类全有主；hh-back 牌背/se-deed/pcp-btn 已豁免勿报）；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
