...
> **第 8 轮更新**。基线：`origin/game-1.3 = 6982da7e`（主干未动；在途重复度恶化——**N-12/N-10/N-3 各双开、N-55 三开、红灯五开**；862b 的 r20 笔记又占 N-108…N-116 但其中 N-108…N-113 **全部与主干 N-100…N-105 重复**，仅 N-114…N-116 是把迭代U-2/3/4 转正）。**引用一律用主干号（水位 N-107）**，新伤勿自编号。
...
| 学习员 862b `c33650c1`（r19 笔记） | N-103…N-107 | 选关地图 UX 族（页签堆高/136px/pan-y/视觉/mapColumns） | **N-103…N-107 现已全被主干占用**，其号全作废；内容已收 §3.1/§五（标〔862b〕） |
| 学习员 862b `5bc04d4a`（**r20 笔记，第 8 轮新收**） | N-108…N-116 | 自建四线对账矩阵后从 N-108 续编 | **进场核对晚于主干定名**：其 N-108/109/110/111/112/113 = 主干 **N-100/101/102/103/104/105**（逐一重复）；仅 **N-114（=迭代U-2 模式键）/ N-115（=迭代U-3 duo-rush 竖屏菜单）/ N-116（=迭代U-4 首页 hero 平板）** 是给旧 U 项转正的新号。若 r20 先合，N-114…116 定名生效、N-108…113 须并回主干号 |
| #2 B `1856baa6` 与 9efa `e32540ba` | N-12 / N-10 / N-3（等） | 前者：台球双栏+台面钳 / 围棋整盘 / 地产真预算+座位收行；后者大杂烩单再修 **N-12/N-55/C-8/N-10/N-3/N-90 判定线** 六伤 | **N-12/N-10/N-3 已双开**——先合版生效，后合方逐伤对照拆账；**C-8 首次被认领**（9efa）；9efa 所称「N-90 判定线」与主干 N-90（tap-tiles 已结案）**同号异伤**，合入须改号 |
...
| #2 B `1c93b984` vs r17B `3c67` vs 9efa `e32540ba` | N-81 / N-55 | B=fixed 钉底；3c67=两列/一行；9efa=雪战十二键再修 | **N-55 已三开**，先合版生效，其余全弃 |
| tester2-9ad5 `7a4f732e` | — | l99 胜负弹层矮横屏按钮进屏（safe center+可滚+z30）+ 320px 弹窗不出右界 | 新伤新修一体，在途；与 N-33/配方 I 弹窗账相邻，合入后回归结算弹窗 |
| 4e78 `6f971780`/`7007612f` | — | l99 钳高只限地图态、舞台可竖滚；连连看/数独内滚；搜索框藏原生双叉；**quiz99 幽灵热区**（inset:0 景片盖抬头条仍收点击，建定位上下文修） | **大杂烩单**：与 N-99/N-100 部分重叠，合并时按伤拆账；幽灵热区是独有新伤（见 §一），无号待统编 |
| b255 `ae99317d` | — | 390 竖屏顶栏攻略钮收图标、游戏名让宽 43→67px；收藏册全屏档补安全区+16/14 红线 | 独有壳层伤面在途；与 #1 A 独占的 styles.css/壳层重叠，A 合并时先 fetch 此单防冲突 |
| 学习优化-C 647a `ea024458`（r2） | U-7…U-12 | 五视口实测增补 + l99-host 滚动军规 + A/B 任务单 | 与本文 U-x 番号撞车持续：其 U-7…U-12 ≠ 本文 §五 U-7/U-8。引用时冠「迭代U-x」，先合版拿正名 |
...
| 学习员 862b r20 `5bc04d4a`（第 8 轮新收） | 四线对账矩阵；自编 N-108…N-116（其中 108…113 重复主干 100…105；114/115/116 = 迭代U-2/3/4 转正） | 在途；消歧见 §3.0，引用先用主干号 |
| 学习优化-C `ea024458`（r2，第 8 轮新收） | U-7…U-12 五视口实测增补 + l99-host 滚动军规 | 在途；U 番号与本文持续撞车，引用冠「迭代U-x」 |

## 七、第 8 轮给 #1/#2 的可勾选执行清单

任务号与独占以主干 r18 playbook（主文+两附录）为权威；**动手前先读 §3.0**。**禁双开**：N-105 红灯（五路在途）、N-99 数独（三路）、N-100 误滚（两路）、**N-12/N-10/N-3（已双开：`1856baa6` + 9efa）**、N-81/N-55（三方案）、C-8（9efa 已认领）。视口红线：**390×844 划到底 + 915×412 不裁切**，切底伤加测 1024×768。

**#1（A 壳+学习）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（四单）：红灯 `46b26d1e`（他路先合则弃）→「root 管理员行」`980945e8` →「密码门」`4970d8ef` →「挑拣车厢」`c2a21b4c`（**勿用 N-94，注明别名待统编**）
- [ ] **N-99 数独 / N-100 误滚**：先 `git log` 对账 4e78/b255 是否已合——已合只回归留数字，未合才动手（§3.1 修法）
- [ ] N-97 math-farm 深关选项 416；迭代U-4 首页平板 hero（首卡 557→≤500，只加档；862b r20 拟定名 N-116）
- [ ] **幽灵热区抽查**（§一 第 8 轮新伤型）：quiz99 系抽 3 款，选关抬头条按钮跑 `elementFromPoint` 断言；4e78 `7007612f` 先合则只回归
- [ ] 〔862b〕壳层滚动排查：`.l99-view` 补 `touch-action:pan-y`；章节页签堆高与 136px 常数（联动 `rootUnlock.ts:108`）
- [ ] A 面结案落账：N-60/61/62/N-90/N-91 回归数字（禁再修）；N-11/N-46/N-56 复测仍开则修
- [ ] U-101 竖屏首扫（只补缺口，b255 顶栏 43→67 若先合带回归）；U-2 横屏左右安全区（`styles.css:261`）
- [ ] 交卷：水位 ≥ 1193/19489 且 N-105 守门 5 红转绿；报告进当轮 tester-A 文件

**#2（B 休闲对战）**——基于 `game-1.3` 最新（≥ `6982da7e`），按序执行：

- [ ] **先交卷压箱货**（三单）：brave-path `a6ed4010` → N-81/N-55 fixed `1c93b984`（三方案先合版生效）→ **N-12/N-10/N-3 `1856baa6`**（与 9efa 双开，先合版生效；合入后补 N-3 平板档 1024×768/1180×820 数字）
- [ ] **N-105 红灯 / N-55 / C-8 / N-12/10/3**：全部勿写新份，先合版落地后跑守门/回归确认
...
- [ ] 迭代U-2 模式键复测（9ad5 先合则只剩 balloon-pop/duo-rush；862b r20 拟定名 N-114）
...
**两位共通**：§3.0 先读；贴线族先查壳层预算（N-89 教训）；sticky 失效先量裁切祖先链；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；**新伤一律只记「文件+数字」勿自编号**——862b r20 在途已占到 N-116 且 N-108…113 与主干重复，统编交学习员/监督裁决；撞车取先合版。
