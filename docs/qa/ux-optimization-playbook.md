...
> **第 19 轮更新**。基线：`origin/game-1.3 = 206d0522`（主干七轮稳）。重灾续报：**N-99 已六路**（1cd5-A `fbc6655b` 又修一版）；**N-128 壳层静态守门与 N-127 已被 1cd5-A 实修在途**——本文原派给 #1 的首位任务转「对账勿双开」。b255 交出 **N-100 扩面 17 款验收数字**（r19 口径+root 走 UI）。号池经 1cd5 r26/r27 推进到 **N-140**，下一空号 **N-141**；新并号裁决：N-139 = mp-open/rbv-foe 40、N-140 = 数独 `.sp-key` 40。水位基准 1202/19513。勿开 N-110…116。
...
- **N-99 数独盘面**：**六路**——5c27 被派 / 4e78 内滚 / b255 模式条+`f8455b6e` 直修 / 9a87 盘可滚+守门 / **1cd5-A `fbc6655b`**——先合版生效，其余全弃
- **N-128 静态守门 / N-127 断点空洞**：**1cd5-A `fbc6655b` 已实修**（连带 N-131、kit N-132/137、block-drop 返回）——#1 原首位任务转「对账勿双开」，合入后只回归
- **N-100 误滚卷顶**：**四路**——5c27 被派 / b255 部分修复+`f8455b6e` 直修 / 9a87「锚定 sticky+钳滚距」——先合版生效；残余页签面 1cd5 N-117 已实修在途
- **N-97 农场深关**：**双开**——9a87 答案钉底 vs b255 `f8455b6e`——先合版生效；N-109 root 门收档仍 9a87 独有（余力项+rootGate 测试）
- **拟名 N-117/N-118/N-120 壳层族**：1cd5 `9e78f25f` 已实修（页签徽章/去 136px 盲区/触摸滚动）+守门 `b2a04f49`——**A 勿再动**，合入后只回归
- **bubble-aim**：双开精确化——e75a **PT-1/PT-2/PT-3**（触摸滚动 `.ba-wrap` manipulation / 平板地图 384→720 八列 / ba-cs 画布 clamp 双条件档）vs 5f46 `f72b937f`+`50a91a5d`+`a703e1f8`（680/820 六列+桩兼容）——**同伤异修，先合版生效**；后合方只保留不重叠选择器；PT-3 的双条件档护住 N-29 矮横屏账，取舍时优先保这条护栏
...
| **#1 e75a `b0a0a22e`（首轮交卷，第 17 轮定档）** | PT-1…PT-4 | `pt-portrait-tablet-r1.md`：PT-1 bubble-aim 竖屏划不动（touch-action）、PT-2 平板地图窄柱（照 candy-swing `cs-view-map` 先例）、PT-3 ba-cs 平板画布 360×480→411×548（clamp 双条件档）、PT-4 candy-swing 竖屏裁 18（预算 −120→−138）；守门 `portraitTablet.pt.test.ts` 8 例；N-105 抢修让位先合。**干净账**：76 款×3 视口真触摸滚动全绿、首页五视口零溢出、暂停面五钮 44、mole-pop 端到端通——**下轮不必重量**。遗留：gold-hook/junqi/dark-chess 平板置顶留白、duo-arena 菜单 43% 宽。引用冠「PT-x」待统编 |
| **9a87 `b7677155`（第 17 轮新收）** | 修 N-99/N-97/N-100/N-109 | 一单四号：数独盘可滚+守门 / 农场深关答案钉底 / l99 进场锚定 sticky+钳滚距 / root 门收档+rootGate 测试——与 4e78/b255 的 N-99/N-100 侧**多路撞车，先合版生效**；N-97/N-109 首认领 |
...
- [ ] ~~N-99~~ **四路在途勿五开**（9a87 已带修+守门，§3.0）——先合版生效，合入后只回归留数字（390 已绿勿回退；题库/seed 零触碰）
- [ ] ~~N-97~~ **在途已修**（9a87 农场深关答案钉底）——先合版生效，合入后 root 永久档直达末章末关回归
...
- [ ] ~~N-100~~ **三路在途勿四开**（9a87「锚定 sticky+钳滚距」/ b255 部分修复 / 5c27 被派，§3.0）——先合版生效；合入后**验收跑 r19 §四 17 款清单**；若先合版仍有残余负 top，按 §八 R-10 诊断后补 R-1 scroll-margin 一行，勿写第三套 JS；N-63 stage 归零勿回退
- [ ] ~~拟名 N-117/N-118/N-120 壳层族~~ **1cd5 在途实修勿双开**（`9e78f25f` 页签徽章/去 136px 盲区/触摸滚动 + 守门 `b2a04f49`）——先合版生效，合入后回归：页签徽章确认 `rootUnlock.ts:108` 锁标未被拍平、地图首屏 ≥2 整行；「设置/选人屏 overflow:hidden 且无矮屏媒体」全仓扫仍归 B 各游戏份
...
| 1cd5 `9e78f25f`+`ee4ebe70`+r23/r24 笔记（第 17 轮新收） | 实修 N-117/118/120（页签徽章/去 136px 盲区/触摸滚动+守门）与 N-124 打靶键排钉底；r23 开 N-129…131、r24 开 N-132…134 | 在途；N-117/118/120 从「等定名」转「在途实修」 |
| b255 `f8455b6e`+`ce522304`（第 18 轮新收） | N-97/N-99/N-100 三项 915×412 直修 + 报告增补修复数字与未修说明 | 在途；**与 9a87 同三号撞满**（N-99 五路/N-100 四路/N-97 双开），先合版生效 |
| 1cd5 `87870e12`+r25/r26 笔记（第 18 轮新收） | r21A 交卷补记——**vitest 水位 1202/19513（新基准）**；r25 开 N-135…137、r26 从 N-138 起并做返回钮并号裁决（dvs→N-94/101、ld→N-104、shr→N-134、rbe→N-133、四款无高度返回→N-135） | 在途；**返回钮类新伤先查 r26/r27 并号表** |
| b255 `12de1e02`+`8d1bcb7e`（第 19 轮新收） | 合主干取先合；**N-100 扩面 17 款验收数字**（r19 口径，root 走 UI 密码门） | 在途；N-100 验收面从此有实测底账 |
| 1cd5 `fbc6655b`+`642541e3`+`1a2be2c9`（第 19 轮新收） | r22-A 修 **N-128/N-99/N-131/N-127** 与 kit N-132/137、block-drop 返回；r23–r25 交卷索引；r21-B 交卷 N-121/122/124 三视口数字 | 在途；**N-128/N-127 从「#1 动手位」转「对账勿双开」**；号池 r26 开 N-138…140、r27 从 N-141 起 |
...
## 七、第 19 轮给 #1/#2 的可勾选执行清单（两位仍在 r2）
...
**#1（A 壳+学习，r2——首轮 PT-1…4 已交卷）**——基于 `game-1.3` 最新（≥ `206d0522`），按序执行：

- [ ] **r1 交卷收尾**：PT 四单+守门 8 例保持可合状态，冲突 rebase 勿 force；bubble-aim 与 5f46 双开——先合版生效，若 5f46 先合则只保留 PT-3 双条件档护栏（N-29 账）
- [ ] **认领潮后零动作清单（重灾升级）**：N-99（五路）/N-100（四路）/N-97（双开）/N-109、N-117/118/120（1cd5 实修）、顶栏挤扁（b255/4e78）——全部只等先合版，碰 = 打回
- [ ] ~~N-128 / N-127~~ **1cd5-A 已实修勿双开**（`fbc6655b`，§3.0）——只对账，合入后回归
- [ ] **r2 空白项（A 真正动手位，按序）**：幽灵热区 R-4 全仓粗筛（`rg "inset:0"` + elementFromPoint 断言）→ 拟名 N-123 首页平板 hero（首卡 557→≤500，只加档）→ **PT 报告遗留**：gold-hook/junqi-camp/dark-chess 平板置顶留白（壳层 opt-in 类方案，勿全局 safe center）、duo-arena 菜单 43% 宽
- [ ] U-2 横屏左右安全区（R-7 照 overlay 先例）；U-101 竖屏首扫**只剩 PT 干净账之外的缺口**（76 款×3 视口触摸滚动已全绿免重量）
- [ ] 交卷：水位只增不减（新基准 **1202/19513**）+ 红灯守门绿；🟡/🔧 分开列；报告进当轮文件

**#2（B 休闲对战，r2 进行中，底座已同步 `abe8ad03`）**——按序执行：

- [ ] **r1 交卷收尾**：模式键/胜负弹层/touch-action/红灯对齐四单保持可合状态，冲突时 rebase 勿 force；合入后迭代U-2 只剩 balloon-pop/duo-rush 复测（1cd5 已修 merge-2048/shoot-range 侧，不同款可共存）
...
**两位共通**：§3.0 先读，碰「已合只回归」项 = 打回；**并号优先于开新，勿开 N-110…116**（主干水位 N-109，在途已占到 **N-140**，下一空号 N-141——但新伤仍只记「文件+数字」交学习员统编）；**返回钮/小热区类伤先查 1cd5 r26/r27 并号表**（N-139 mp-open/rbv-foe、N-140 数独 sp-key 已有主）；初见出屏先 scrollIntoView 再定性；门类流程回归走真 UI；贴线族先查壳层预算；救济档四视口覆盖、加档不改档；矮屏收缩红线 §6.6 逢改必查；撞车取先合版；**修 N-100/N-103/N-108/N-124 前先读 §八对应配方**。
