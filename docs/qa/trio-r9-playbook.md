...
> **只列仍未落地的 🔧**。r4–r8 原编号继续有效，本轮新发现从 **N-39** 续编（蓝本地图聚焦）；补抽工位另加 **N-42**（收藏册 overlay 跨路由）、**N-43**（数独对战竞速）。
> 分工建议：**A = 壳层 + 闯关学习**，**B = 休闲对战动手（先收自己在途分支）**。
> 修补配方 A/B/C 见 r4 learn-notes 第六节，D/E/F 见 r5，G/H 见 r6，I/J 见 r8，**本轮新增 K（共享蓝本的机制要看接线）与 L（门户绑路由）见 r9 learn-notes 第四节**。
...
- 量法口径沿用：stage 裁切（r4）→ 自滚容器（r5）→ 逐模式（r6）→ 开关打开再量（r7）→ 弹窗内滚与够不着的钮（r8-I）→ 深关关型矩阵（r8-J）→ **调用路径 × 默认参数（r9-K）**。
...
- 改哪照 r8（矮横屏 root 行并一行或折叠;quiz 宿主高度把 root 行算进去）。**验收矩阵扩成：4 quiz 皮肤 × root 开/关 + 深关限时档（pinyin 135 关族）+ math-farm 第 151 关族（无 root 三钮 top 439 也线下）**;工装按「每档隔离 context」红线执行。

> S-1 / S-2 / S-3 ✅ 已由 A 第 2 轮销账（复证数字见 r9 learn-notes 第二节），从本清单除名。

### N-42 收藏册 overlay 跨路由残留 🔧（先做，走配方 L；与 N-39 不同账）

- **现象**：开 🎁 后改 hash 进游戏，`.collection-overlay` 仍在（z-index 60）。
- **改哪**：`src/ui/collection.ts` 打开时挂 hashchange → close，close 里摘监听。试穿 canvas / 钱包 key 零触碰。
- **验收**：开收藏 → `#/game/clock-house` → overlay 0；Esc/关闭钮原路径仍关。补一条对照 `parentAuthRoute.test.ts` 的测试。可与收藏册 40/36 热区同 PR。
...
- r6/r7 原文原样有效。**补抽抽核**：915 开触屏键后裁 **498** / canvas 出屏 **335** / 轻击·重击·必杀·防御 + ⏸ 全线下。**390 开触屏后裁 0**（当护栏，勿回退）。照 r7 playbook,可与 N-31 同 PR。

### N-43 sudoku-petal 对战竞速 🔧（新，走配方 H + G）

- **现象**：进「🤝 对战竞速」后 915 裁 1046；412×915 裁 519；390×844 裁 590 —— 棋盘主体折叠线下。
...
- 护栏（勿扩大化）：fk 无尽连胜、dvs 五模式、壳层暂停/家长门弹窗、l99 过关/失败覆盖层、攻略抽屉两形态、组字/限时基线、首页搜索态——干净名单见 r7/r8/r9 learn-notes。
...
3. 本轮新伤 N-39 / N-42 / N-43 与 N-37 加重档在 915×412 复测截图或数字留档;N-39 验收样本含 hop-pads(头部最高);N-42 必须先关 overlay 再量别的;「开/关对照」验收一律每档隔离 context。
