...
> **不要改**：fruit-stack 同名 `.fs-back`（已绿）；`.oa-open` / `.oa-btn` / 技能；`.fs-act` / `.fs-open`。
...
playbook 基线无 min-height；本分支已有 `min-height:44px`。补 `display:inline-flex; align-items:center; justify-content:center; box-sizing:border-box`。不改 `.oa-open`。

### N-202 `src/games/fishing-star/index.ts` `.fs-back`

只改钓鱼之星。fruit-stack `.fs-back`（粉 `#a8456a`）一字不改。暂停 `.fs-btn` 一字不改。
...
`npx vitest run src/games/hotspot.r47b.test.ts src/games/hotspot.r22b.test.ts`

## 820 档 / N-105

不回退。combo-clash 无 820。
