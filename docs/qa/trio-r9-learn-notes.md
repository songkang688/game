...
>
> **补抽工位（隔离 worktree `cursor/trio-r9-learner-c14c`，基线后快进到 `4b3a4cab`）**：不改已合入的 N-39 / 配方 K。另开端口 4176 补测 r8 点名但首份笔记没量的面（2P 手柄、失败/超时结算、横屏键盘、收藏册试穿、家长导出、`isDrawLevel` / 深关 math-farm / 对战竞速）。新号从 **N-42** 续。
...
**补抽工位新发现（N-42 起；N-39 不动）**：

| # | 对象 | 实测 | 性质 |
| --- | --- | --- | --- |
| N-42 | **收藏册 overlay 跨路由残留** | 首页开 🎁 后 `hash = "#/game/clock-house"`，`.collection-overlay` 仍在（`z-index:60`），舞台已 mount。`collection.ts` 无 hashchange（S-3 只修了家长门）。试穿预览本身 915 完整（canvas 136–366），热区仍 40/36 | 🔧 壳层，配方 L |
| N-43 | sudoku-petal **对战竞速** | 进「🤝 对战竞速」后 915 裁 **1046**（~147 格 top≥416）；412×915 裁 519；390×844 裁 590。C-1 藏条 ≠ 玩法态绿（配方 H 漏这一跳） | 🔧 |

**补抽抽核（不开新号）**：N-25 开触屏键后 915 裁 **498** / 出屏 **335** / 轻击·重击·必杀·防御·⏸ 全线下（与 r6 一字不差）；390 开触屏后裁 **0**（当护栏）。N-1 915 **741/617**、1024 **415/281** 一字不差。N-30 古堡 915 裁 348、8 控件线下；390 裁 0（ak-bar 销账顺带）。duo-rush 2P 垫 915 在屏（333–377）。shape-kingdom `isDrawLevel` 第 102 关族 915 已有 `shk-fit-scroll`+`pan-y`，不立项。pinyin 限时 **超时 overlay** 78–406、钮 top 320 在屏。math-farm 第 151 关族无 root 三钮 top 439 线下——并进 N-37 验收（「章节换皮」无 `isXxxLevel` 也是关型）。
...
### 模式 L · 壳层自制门户必须绑路由（N-42，S-3 的同胞）

- **机理**：`position:fixed; z-index:60` 的收藏册不是 `.dialog`，也不走 `parentAuth` 的 hashchange。路由变了节点还在，等于第二块屏幕盖住关内；后续量测会被这块污染（本工位后半段曾中招）。
- **配方**：对照 `parentAuth.ts` 的 `finish(false)` —— `openCollection` 挂 hashchange → `handle.close()`。存档 key、试穿绘制零触碰。
- **修好判据**：开收藏 → 改 hash 进任一游戏 → overlay 0。
...
| `frontend-design` | 门户组件的生命周期包括「路由走了也要关」；收藏册试穿是另一屏，热区仍要 44px | N-42 / 配方 L |
| `canvas-design` | 对战盘不能当文档流：数独对战三档视口都把棋盘滚出屏 | N-43 |
...
7. 补抽结案：duo-rush 2P 手柄 915 在屏；shape-kingdom 作图关 915 有逃生门；限时超时 overlay（宿主不裁时）干净；N-25/N-1 数字抽核仍准。

## 七、下一轮 A/B 最该先做的 10 条（与 r9 playbook 排序一致）

1. **N-33 壳层结算弹窗矮横屏必点钮线下**（共享件一处修 76 款受益,连续两轮最重;正面样板 = 攻略抽屉 sheet）
2. **N-42 收藏册 overlay 绑路由**（半小时级，先做免挡住后续验收；与 N-39 不同账）
3. N-25 fight-king 格斗塔（**抽核 498/335 仍在**；390 开触屏已绿当护栏）
4. **N-39 蓝本地图聚焦当前关**（一行级 × 4 调用点）+ **N-43 数独对战竞速**（三档都中）
5. N-34 拼写关 + N-35 全选关（一个 PR）
6. N-1 fruit-catch（抽核 741/617 仍在；先看 B 在途是否已覆盖）
7. N-30 无尽古堡 915（390 已绿；B 在途先对账）
8. N-36 描红（抄窗口 8 作图关的 fit 记号）+ N-37（含限时 135 与 math-farm 151 深关）
9. **N-38 + qz-38 + 收藏册 40/36**（半小时拉绿；可与 N-42 同 PR）
10. L-2 / L-3 + N-26/N-27 + 守门测试二合一
