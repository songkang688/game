# 1.2 第 33 步 / 共 37 步 · B 档 —— `dark-chess`「翻翻暗棋」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 33 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `dark-chess` 🀄️「翻翻暗棋」。
...
必须通读：`src/games/dark-chess/` 下 `src/games/dark-chess/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
**不是升级 xiangqi。** 揭棋翻子必须有翻转过程。

当前 1.1 登记模式：B 用本款补象棋名额（揭棋）。
本步模式目标：精细化：翻子、吃子规则、多档、翻开动画。
...
## 独占该游戏目录
只许修改 `src/games/dark-chess/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
