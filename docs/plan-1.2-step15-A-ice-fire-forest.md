# 1.2 第 15 步 / 共 37 步 · A 档 —— `ice-fire-forest`「冰冰火火森林」

> 本文件是 **C 档升级提示词**（一步一档一文件）。只升级这一款。禁止实现范围外的游戏，禁止再派生云端子代理写代码。
> 同一步另外两档：见 `docs/plan-1.2-step15-*.md`。步号从 9 连续到 34 为升级，35–37 为三角色验证。不要改 `docs/game-1.2/00-*`、`step-01.md`、`new-games/`。
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 15 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `ice-fire-forest` ❄️「冰冰火火森林」。
...
必须通读：`src/games/ice-fire-forest/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已凛凛/焰焰、188 可解、Tab 单人。精细化：关卡阅读性、2.5D 远景可选、机关动画过程（门打开要滑，不是瞬开）。

当前 1.1 登记模式：campaign 188 / coop / twoPlayer。
本步模式目标：可加无尽机关跑；单人 Tab 保持。
...
## 2.5D / 3D 决策
2.5D **可选**：若能提升阅读性，用共享 `view25d` 做浅透视；默认仍可 2D。禁止 three.js，禁止为 3D 而 3D。
跑酷类在 1.1 第 6 步基础上继续；大鱼吃小鱼加无尽；不要全局 3D。

## 视觉动画
2D 为主，可选浅透视。无死亡描写（掉岩浆=送回检查点+鼓励）。
...
## 独占该游戏目录
只许修改 `src/games/ice-fire-forest/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
