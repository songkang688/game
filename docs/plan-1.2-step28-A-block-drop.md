# 1.2 第 28 步 / 共 37 步 · A 档 —— `block-drop`「方块叠叠乐」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 28 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `block-drop` 🧱「方块叠叠乐」。
...
必须通读：`src/games/block-drop/` 下 `src/games/block-drop/**`。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
接入后检查踢墙、7-bag、hold。消行后上方块必须逐格下落，禁止瞬移。

当前 1.1 登记模式：B 接入 SRS/7-bag。
本步模式目标：精细化：幽灵块、DAS、消行动画（行必须有下落过程）、对战垃圾行。
...
闯关 188 若有则保持；无尽马拉松；对战垃圾行。文案禁用俄罗斯方块/Tetris。
...
## GitHub 结构参考
sen-ltd/tetris 的 SRS/7-bag/hold/ghost（禁 Tetris 商标进文案）
只学结构 / 状态机 / 测试切法。不抄商标素材、不引入运行时依赖、不搬协议代码。

## 独占该游戏目录
只许修改 `src/games/block-drop/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
