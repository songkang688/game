# 1.2 第 15 步 / 共 37 步 · C 档 —— `prince-princess`「王子公主大冒险」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 15 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `prince-princess` 🤴「王子公主大冒险」。
...
必须通读：`src/games/prince-princess/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已王子近战公主远程二段跳、共享心条、188、14 Boss、Tab 单人。精细化：共享条改叫元气、Boss 读招、窄屏两人同屏。

当前 1.1 登记模式：campaign 188 / coop / twoPlayer / endless。
本步模式目标：关卡阅读性、共享元气条。
...
## 独占该游戏目录
只许修改 `src/games/prince-princess/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
