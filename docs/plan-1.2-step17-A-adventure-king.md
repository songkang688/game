# 1.2 第 17 步 / 共 37 步 · A 档 —— `adventure-king`「冒险小王」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 17 步 / 升级共 26 步（总派发到第 37 步）**，角色 **A**，独占 `adventure-king` 🗺️「冒险小王」。
...
必须通读：`src/games/adventure-king/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已回旋镖抓钩三神器 188。精细化：地图标记、神器动画、无尽遗迹、跳关直达。
...
## 独占该游戏目录
只许修改 `src/games/adventure-king/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
