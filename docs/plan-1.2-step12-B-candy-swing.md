# 1.2 第 12 步 / 共 37 步 · B 档 —— `candy-swing`「糖果秋千」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 12 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `candy-swing` 🍬「糖果秋千」。
...
必须通读：`src/games/candy-swing/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已 188 关。精细化：绳物理稳定、剪绳要有断裂过程、360px 指尖热区、星星收集动画。
...
## 视觉动画
2D。绳是线不是 3D。reduced-motion 仍要断裂两段位移。
...
## 独占该游戏目录
只许修改 `src/games/candy-swing/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
