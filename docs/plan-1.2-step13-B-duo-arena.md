# 1.2 第 13 步 / 共 37 步 · B 档 —— `duo-arena`「朵星擂台」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 13 步 / 升级共 26 步（总派发到第 37 步）**，角色 **B**，独占 `duo-arena` 🥊「朵星擂台」。
...
必须通读：`src/games/duo-arena/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 是上下半场点点/金币/炸弹三回合。精细化：平衡、道具可读、人机档、无尽连胜。不要做成 fight-king。

当前 1.1 登记模式：versus / twoPlayer（点点大战，无关卡表）。
本步模式目标：可加无尽连回合；不要无关卡凑 188。
...
## 独占该游戏目录
只许修改 `src/games/duo-arena/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
