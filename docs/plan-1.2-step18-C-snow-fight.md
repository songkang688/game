# 1.2 第 18 步 / 共 37 步 · C 档 —— `snow-fight`「雪球大作战」
...
仓库 https://github.com/songkang688/game ，产品「一朵一星」，**1.2 升级第 18 步 / 升级共 26 步（总派发到第 37 步）**，角色 **C**，独占 `snow-fight` ⛄「雪球大作战」。
...
必须通读：`src/games/snow-fight/` 下 目录内全部。对照 `meta.ts` 的 `modes` / `levels` / `blurb` 与真实玩法是否一致。
1.1 已蓄力风向落点、掩体、188、人机三档、雪怪车轮战。精细化：抛物线可视化、雪墙堆砌过程、命中蓬松炸开不是伤害。

当前 1.1 登记模式：campaign / versus / twoPlayer / endless。
本步模式目标：投掷手感。
...
## 独占该游戏目录
只许修改 `src/games/snow-fight/**`（含测试）。`meta.ts` 可改以符合事实。公共框架（`level99.ts`、`src/ui/*`、`src/engine/*`、`src/styles.css`、`index.html`、其他游戏）**只读**。不要改 `docs/game-1.2/00-*`、`new-games/`、旧 `upgrades/`、第 1 步文档。
